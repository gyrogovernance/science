...
"""
High-level Holonomic Quantum Computing SDK surface for the Gyroscopic ASI hQVM.
...
- Native hQVM runtime entry points (RuntimeOps over tools.gyroscopic.ops)
...
def compare_ledgers_pair(
    ...
    common = 0
    upper = min(len(l_bytes), len(r_bytes))
    while common < upper and l_bytes[common] == r_bytes[common]:
        common += 1

    if len(l_bytes) == len(r_bytes) and common == len(l_bytes):
        return 0, common

    if common == len(l_bytes):
        return -1, common
