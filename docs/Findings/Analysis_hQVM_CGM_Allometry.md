...
Biological allometry concerns power-law scaling of physiological rates and times with body mass. This analysis derives a closed band of metabolic scaling exponents between `2/3` and `3/4`, together with related quarter-power timing exponents, from a fixed six-mode transport geometry in the Common Governance Model (CGM). CGM is an axiomatic framework for fundamental physics and information science resting on the requirement that every distinguishable state in a coherent system trace to a common source.

The unifying quantity is a source-accessibility exponent `a`, the logarithmic slope of how source-connected delivery capacity grows with organism size. An organism is modeled as a source-to-bulk transport system. The transport geometry is supplied by a finite executable kernel, the Holonomic Quantum Virtual Machine (hQVM): a reversible 4096-state state space whose transport register has six binary modes, corresponding to three rotational and three translational degrees of freedom.

Three delivery regimes produce three canonical exponents. A kernel product-geometry floor, in which reachable bulk capacity grows as the square of the boundary set of states directly connected to the source, gives exponent `1/2`. Pure surface exchange in three spatial dimensions at constant density gives exponent `2/3`. An internal delivery network gives exponent `3/4` when its effective dimension is set by the kernel's closed-form equilibrium occupation over chirality shells, recovering the Kleiber relation. A continuous flux-fraction parameter measures the share of metabolic throughput carried by the network channel at fixed dimensionality and interpolates between the surface and network endpoints `2/3` and `3/4`.

Internal consistency constructions on the family of generalized carriers uniquely select the six-mode transport dimension. Separately, the same small kernel aperture parameter used in the CGM gravity construction yields a chemical activation-energy scale of 0.645 electron volts at mammalian body temperature, inside the independently measured 0.6--0.7 electron-volt Metabolic Theory of Ecology band. Development and life-history coverage, and city and company scaling, are reported as lower-confidence structural interpretations distinct from the organism channel basis.
