...
3. The foundational conditions hold as expectation values under ω.
...
1. **Axiomatization** (Z3 SMT verification): Establishes logical consistency, independence, and entailment structure of the foundational conditions via Kripke frames.
...
3. **3D/6DoF Derivation** (Lie-theoretic proof): Proves that the foundational conditions uniquely determine n=3 spatial dimensions and d=6 degrees of freedom via BCH conditions, simplicity requirements, and gyrotriangle closure.
...
Define ω: A → ℂ as a positive linear functional with ω(I) = 1. The functional encodes the foundational conditions:
...
- Proposition BU-Ingress: The balanced state (depth-4 closure) implies reconstruction of prior conditions (CS, UNA, ONA).
...
- Balance implies reconstruction: the conditions ensure prior properties hold under the balanced state.
...
**Conclusion:** The t² term vanishes in the S-sector, leaving O(t³) conditions that force su(2) structure. This confirms the theoretical prediction and validates the 3D derivation.
...
On S¹ (1D manifold, 2D embedding), the foundational conditions impose contradictory requirements:
...
- Conclusion: 2D fails under RA+BCH conditions that uniquely select su(2) in 3D

This exclusion is independent of the SO(2) abelian obstruction, providing a second, analytic proof that n=2 cannot satisfy the foundational conditions under unitary representation.
...
3. **Proposition BU-Egress (depth-4 balance)** → **BCH conditions** on commutators
