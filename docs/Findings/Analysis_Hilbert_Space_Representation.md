...
### 3.2 Encoding Axiom CS3 (Balance is Universal)

Axiom CS3 states □B, meaning the four-step compositions commute. We encode this as:
...
### 3.3 Encoding CS1 and CS2 (Non-Absoluteness at Depth Two)

CS1 states ¬□E (two-step equality is not absolute). We do not set u_L u_R = u_R u_L as an operator equality. Instead:
...
CS2 similarly prevents absolute inequality.

### 3.4 Chirality Encoding (CS6, CS7)

CS6 states [R]S ↔ S, meaning right transition preserves the horizon:

```
ω(u_R) = ω(I) = 1
```

CS7 states ¬([L]S ↔ S), meaning left transition alters the horizon:
...
- ⟨Ω|U_L U_R U_L U_R|Ω⟩ = ⟨Ω|U_R U_L U_R U_L|Ω⟩ (CS3 encoded)
...
**Non-commutation at depth two (CS1, CS2):**
```
...
### 8.2 Axiom CS6 (Right Preservation)
...
The joint spectrum of (U_L, U_R) encodes the complete modal structure. The balance condition CS3 constrains this joint spectrum, requiring:
