...
Holonomy is the residual transformation that remains when a system is transported around a closed path in a curved space. The traversal returns to its starting point, while the orientation carried along the path does not. This document establishes the holonomy structure of the Common Governance Model (CGM) in three layers. The first layer contains exact algebraic results that follow from the CGM threshold angles alone, including a closed form for the SU(2) commutator holonomy. The second layer declares an explicit embedding of the CGM stages into a gyrovector space and derives the BU dual-pole holonomy in closed form as an elementary function of two thresholds, from which the closure ratio ρ ≈ 0.9793 and the aperture gap Δ ≈ 0.0207 follow as definitions. The central structural result of this layer is a conjugation theorem. The palindromic traversal of all payload stages preserves the holonomy angle while transporting its axis, which separates the magnitude of path memory from its orientation. The third layer verifies the finite realization of the same architecture in the Holonomic Quantum Virtual Machine (hQVM), where holonomy appears as an order-two operator structure on a 4096-state manifold. Each result carries an explicit status label distinguishing exact algebra, embedding-dependent derivation, numerical cross-check, and structural correspondence.
...
The first holonomy result requires no embedding. It follows from the threshold angles and the algebra of the group SU(2), the double cover of the rotation group.
...
The embedding-dependent results in later sections are computed with a software implementation of the gyration operator. Before that implementation is used at the CGM threshold coordinates, it is validated against an independent analytic standard.
...
The slope error scales with the square of the speed bound (measured order 2.003) and the absolute residual scales with the fourth power (measured order 4.006). Both orders match the known series structure of the Wigner angle, so the implementation reproduces the analytic behavior across the tested range rather than at a single tolerance point. The empirical noise floor of the matrix layer at the coordinate magnitudes used below is about 10⁻⁸, and all matrix-layer tolerances in this document derive from that floor.

---

## 5. The Declared Embedding

Computing a loop holonomy for the CGM stages requires placing the stages at definite coordinates in the gyrovector space. This placement is a modeling choice, and the analysis states it explicitly so that every downstream number can be traced to it.

The embedding, named the payload scalar embedding, assigns coordinates in the Einstein gyrovector model with c = 1 as follows.
...
Three features of this map require comment. First, BU appears as a pair of opposite poles on the third axis, reflecting the dual character of the balance condition. Second, the ONA threshold, which is defined as an angle, enters here as a coordinate magnitude. This is a declared modeling choice, and quantities derived from the embedding inherit it. Third, CS is absent from the coordinate list. The CS condition supplies the reference frame within which the other stages are defined, and a reference frame is not a location that transport visits. This treatment also keeps every embedded vector inside the open unit ball, since the CS threshold π/2 exceeds 1 and could not serve as a velocity coordinate. The script enforces the ball constraint as a hard gate and confirms that all four payload vectors satisfy it.

The four embedded stages are called the **payload stages**, and CS is called the **gauge frame**. Section 10 shows that this split matches the structure of the finite machine, where an 8-bit instruction consists of 6 payload bits framed by 2 gauge bits.

---

## 6. The BU Dual-Pole Holonomy in Closed Form

The central loop of the analysis visits the ONA stage, crosses to the positive BU pole, crosses to the negative BU pole, and returns.

```
ONA → BU+ → BU- → ONA
```

Under the declared embedding, this loop consists of two gyration corners joined by a pole crossing. The pole crossing contributes no rotation because BU+ and BU- are collinear, so the holonomy is generated entirely at the two ONA-BU corners. Each corner is a gyration of two boosts, one of magnitude π/4 and one of magnitude m_a, separated by a right angle. The Wigner angle for boosts of unequal magnitudes β₁ and β₂ separated by an angle θ is
...
Evaluated at 80-digit precision,
...
This closed form upgrades the status of δ_BU. Earlier treatments obtained the value from floating-point rotation matrices and could report it only as a reproducible numerical result. The value is an elementary function of two thresholds, θ_ONA and m_a, computable to arbitrary precision, and exact under the declared embedding.

The rotation axis of the loop is the direction ONA × BU, which under this embedding is the negative x axis. The holonomy therefore consists of a scalar angle together with an oriented axis, and Section 9 shows that these two components behave differently under transport.
...
The response to m_a is close to linear, with the excess above 1 accounted for by the finite-amplitude correction of Section 7. The response to the ONA threshold is superlinear. Finite-difference derivatives of the matrix realization match derivatives of the analytic formula to a few parts in 10⁴, consistent with the noise floor.

---

## 11. The Wigner Map at the Canonical Thresholds

A separate question concerns the equal-speed Wigner rotation evaluated at the UNA and ONA thresholds, with u_p as the common boost speed and θ_ONA as the separation angle. The resulting angle is compared to the aperture scale m_a. These are independent quantities and no identity between them is assumed.

```
ω(u_p, θ_ONA) = 0.2155499101533235
m_a            = 0.1994711402007164
offset         = 0.0160787699526072   (8.06 percent of m_a)
```

The offset is reported as a structural datum of the threshold configuration. The local geometry of the map at this point admits closed forms. The partial derivatives of the Wigner angle with respect to the boost speed and the separation angle evaluate to
...
and their sum equals 1 as an algebraic identity at this specific point. The response of the Wigner angle at the canonical thresholds splits into a boost-magnitude share of 76.3 percent and an angular share of 23.7 percent, with the two shares closing to unity. The numerical derivatives match the closed forms to better than 10⁻⁴⁰ at working precision.

The parameter values that would bring the Wigner angle onto m_a are also computed. Holding the angle fixed, the required speed is β* = 0.6853, which is 96.9 percent of u_p. Holding the speed fixed, the required angle is θ* = 0.7189, which is 91.5 percent of θ_ONA. A linear prediction of β* from the exact derivative lands within 7 × 10⁻⁴ of the root, confirming that the map is smooth at this point and the offset is an ordinary displacement in parameter space.

---

## 12. Precision Governance

The angle δ_BU feeds downstream analyses, including the fine-structure derivation in Analysis_Fine_Structure.md, where the leading expression scales as the fourth power of δ_BU. A relative change ε in δ_BU therefore produces a relative change of about 4ε in that expression. At the precision levels claimed downstream, the choice of the canonical δ_BU value matters.

The analytic closed form of Section 6 serves as the single source of truth. The shared constant BU_HOLONOMY_ANGLE in the repository constants module is defined from this closed form, and the script verifies that its own 80-digit evaluation matches the shared constant with zero relative difference. Downstream modules import the shared constant rather than embedding literal decimals, which removes the possibility of silent divergence between copies of the value.
...
One caution accompanies the cross-layer numerology. The ratio of the byte-level fold disagreement fraction (0.5) to the continuous aperture gap Δ is about 24.2, and this compression figure appears in the wavefunction analysis. The finite kernel imports the continuous aperture value rather than deriving it, so the comparison is a scale correspondence between layers and not an independent finite derivation of Δ. The script labels it accordingly.

---

## 14. Continuous-Finite Correspondence

The continuous and finite layers realize the same architecture at different formal levels. The following table records the correspondence. The rows identify analogous structural roles and do not assert equality between mathematical objects. In particular, δ_BU is a continuous angle and W2 is a finite operator, and no numerical map between them is derived in this document.
...
The continuous BU holonomy carries a general rotational phase with eigenvalues 1 and exp(±i δ_BU). The finite carrier operators, under their permutation-matrix lift, carry an order-two phase distinction with eigenvalues in {+1, −1}. The correspondence identifies shared architectural roles rather than a shared representation.

The layers also differ in where they localize curvature. In the continuous realization the holonomy is generated at the two ONA-BU corners while the pole crossing is flat. In the finite realization the fold disagreement is counted at the central BU boundary of the byte. Both descriptions organize a dual return through the balance stage. A local equality between the continuous corner curvature and the finite fold disagreement requires an additional derived map.
...
Two numerical proximities are recorded without any claim of identity. They are quarantined from the foundational results and carry no weight in the conclusions.

The SU(2) commutator angle is close to three times the BU dual-pole angle. The residual

```
W = φ_SU2 - 3 · δ_BU = 0.0018742278810340
```

is 0.32 percent of φ_SU2. No derivation connecting the two quantities is claimed.

The BU dual-pole angle is close to the dyadic fraction 50/256, with relative difference 1.5 × 10⁻⁴. The aperture gap Δ differs from 5/256 by 5.6 percent relative, so an earlier informal identification of Δ with 5/256 is withdrawn. If a dyadic quantization attaches anywhere, the sharper candidate is δ_BU, and this remains an open observation.

---

## 16. Result Status

Each result of this analysis carries one of the following status labels.

| Result | Status | Depends on |
|---|---|---|
| stage angles sum to π | exact algebraic | threshold definitions |
| Q_G · m_a² = 1/2 | exact algebraic | threshold definitions |
| φ_SU2 closed form | exact algebraic | SU(2) and threshold angles |
| Thomas-Wigner calibration and convergence orders | analytic standard, numerically verified | gyration implementation |
| δ_BU closed form | exact under the declared embedding | θ_ONA, m_a, orthogonal boosts |
| matrix loop realization | numerical cross-check | declared embedding |
| palindromic conjugation | gyrogroup theorem, numerically verified | inversion identity and SO(3) conjugation |
| reversal, re-rooting, rotational covariance | numerical invariance | SO(3) realization |
| ρ and Δ | derived definitions | δ_BU and m_a |
| small-amplitude expansion of ρ | exact under the declared embedding | closed form |
| rank-two dependency | exact under the declared embedding | closed form |
| Wigner derivatives summing to 1 | exact algebraic | β = u_p, θ = π/4 |
| W2 and W2' certificate | exact finite | hQVM transition rule |
| fold distribution 16 · C(4, k) | exact finite | byte fold algebra |
| continuous-finite correspondence | structural | architectural analogy |
| φ_SU2 versus 3δ_BU, δ_BU versus 50/256 | open comparison | none claimed |
...
2. An independent implementation of the declared embedding yields a δ_BU that differs from 4 · arctan(k(π/4) · k(m_a)) beyond its stated numerical floor.
3. The loop holonomy angle changes under path reversal, cyclic re-rooting, or a global rotation of the embedded points.
...
6. A future claim of numerical identity between a continuous angle and a finite operator spectrum is asserted without a derived map.
...
The first command runs all 37 integrity checks, prints the full report, and writes `experiments/cgm_holonomy_analysis_results.txt`. The run is deterministic, uses 80-digit arithmetic for the analytic layer, and exits with a nonzero code if any check fails. The second command reproduces the finite certificate independently.

---

## 19. Conclusion

Three results form the foundation established here. The CGM threshold angles generate a nontrivial SU(2) commutator holonomy with the exact closed form 2 · arccos((1 + 2√2)/4). Under a single declared embedding, the BU dual-pole loop produces the holonomy angle δ_BU = 4 · arctan(k(π/4) · k(m_a)), an elementary function of two thresholds, from which the closure ratio of 97.93 percent and the aperture gap of 2.07 percent follow as definitions, with the gap decomposing into a 2.98 percent baseline fixed by the ONA geometry and a 0.91 percentage point closure supplied by the finite BU amplitude. The palindromic traversal of the payload stages conjugates this holonomy, preserving the angle while transporting the axis, so the magnitude of path memory is set at the balance stage and its orientation is steered by the surrounding stages.

The finite machine realizes the same architecture in exact arithmetic, with byte-level fold curvature distributed binomially and the balance-stage exchange operators verified as involutions on the full state manifold. The correspondence between the two layers is structural, and the places where it is a scale comparison rather than a derivation are labeled as such. The quantities established here, in particular δ_BU, ρ, and Δ, are the fixed inputs that downstream analyses of physical couplings consume.