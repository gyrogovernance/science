...
The CGM fiber-bundle structure of the byte provides multiple classification axes ([hQVM Specs Formalism](docs/Gyroscopic_Computational_Theory/Gyroscopic_ASI_Specs_Formalism.md), Sections 2 through 4).
...
**Channel isotropy.** From rest, H1(p) is the Shannon entropy of the one-step next-state distribution when a byte is chosen uniformly from the allowed generator set. The full generator set gives H1 = 7 (Features 77 through 79 in [hQVM Features Report](docs/Gyroscopic_Computational_Theory/hQVM_Features_Report.md)).
...
Bytes x and y commute on Omega if and only if q6(x) = q6(y) (Features 68 through 76 in [hQVM Features Report](docs/Gyroscopic_Computational_Theory/hQVM_Features_Report.md)). Exhaustive enumeration yields 1024 commuting ordered pairs out of 65536 (rate 1/64).
