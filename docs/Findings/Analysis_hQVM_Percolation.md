...
The CGM fiber-bundle structure of the byte provides multiple classification axes (`docs/Gyroscopic_Computational_Theory/Gyroscopic_ASI_Specs_Formalism.md`, Sections 2 through 4).
...
The hQVM 24-bit carrier unpacks to a 2 x 3 x 2 tensor with six oriented dipole pairs (`docs/Gyroscopic_Computational_Theory/Gyroscopic_ASI_Specs_Formalism.md`, Section 2.1). Three pairs in Frame 0 correspond to rotational generators of se(3). Three pairs in Frame 1 correspond to translational generators. CGM derives six operational degrees of freedom at the ONA stage as the se(3) Lie algebra SE(3) = SU(2) semidirect R^3. Each payload bit flips one dipole pair, executing a discrete pi-rotation around the corresponding generator.
...
The four holonomic gate bytes HORIZON_GATE_BYTES = {0xAA, 0x54, 0xD5, 0x2B} are the unique bytes that preserve every complement-horizon state and every equality-horizon state in one step (Feature 121 in `docs/Gyroscopic_Computational_Theory/hQVM_Features_Report.md`). This census identifies the set of one-step horizon stabilizers on all 64 states of each horizon.
...
**Channel isotropy.** From rest, H1(p) is the Shannon entropy of the one-step next-state distribution when a byte is chosen uniformly from the allowed generator set. The full generator set gives H1 = 7 (Features 77 through 79 in `docs/Gyroscopic_Computational_Theory/hQVM_Features_Report.md`).
