...
### 5.5 Information architecture relative to Shannon

Shannon's communication theory is organized around a bipartite channel. An input alphabet X and an output alphabet Y are joined by a conditional law p(y|x), and the fundamental quantities are mutual information I(X;Y), equivocation H(X|Y), and capacity C = max I(X;Y). The QuBEC realizes that bipartite architecture in two concurrent places. On the carrier, the state is the pair (A₁₂, B₁₂) with exact complementarity geometry (§1.1, §1.4). On transport, an input distribution on the 256-byte alphabet induces an output law on chirality χ ∈ GF(2)⁶, and thence on shells and climate observables. The kernel transition itself does not require a ledger. Provenance storage is an optional audit surface (§6.1), not part of the channel law.

**Discrete channel and spectral dual.** Shannon analyzes linear invariant filters by Fourier methods and treats sequential composition through convolution. In QuBEC, byte-induced transport on the chirality register is exact XOR translation χ′ = χ ⊕ q (§6a.1). Sequential composition is XOR-convolution on GF(2)⁶, and the Walsh-Hadamard transform is the unique spectral dual of that abelian transport group (§12.2, §16.1). The continuous Fourier integral is replaced by the exact 64-point WHT (§12.4). The byte alphabet has absolute width log₂(256) = 8 bits per symbol. After projection to transport classes, the additive charge lives in GF(2)⁶ and carries at most 6 bits of q-information per step.

**Source entropy and finite typicality.** Shannon's source entropy is a rate obtained in the long-block limit, and his typical-set arguments are asymptotic. QuBEC replaces that limit by verified finite saturation of the future cone (§8.3). From rest, H₀ = 0, H₁ = 7, and H_n = 12 bits for every n ≥ 2. Under the uniform byte ensemble, shell equilibrium is reached in one averaged step and full occupancy of Ω is reached in two steps, so the Shannon typical set of the saturated process is realized exactly as the full manifold rather than as an asymptotic high-probability subset.

**Occupation ensemble and maximum entropy.** Shannon's statistical-mechanical reading of information uses Gibbs ensembles whose partition functions are exponential and whose exactness waits on a thermodynamic limit. QuBEC occupation is controlled by the polynomial partition function of §2.1 and by the exact one-cell Shannon entropy of §5.1. The occupation parameter λ is the native control variable. The shell law is binomial on six independent binary modes, and the maximum-population shell is the equatorial shell of 1280 states (§1.3). No thermodynamic limit is required for these identities.

**Data processing as quotient coarsening.** Shannon's data-processing inequality states that deterministic post-processing of a channel output cannot increase mutual information. QuBEC builds that discipline into the medium as the natural quotient chain of §23.2, namely 256 → 128 → 64 → 7. Each rung discards a definite symmetry while preserving the observables native to that rung. Sector-identification tasks are therefore answered on the appropriate quotient rather than by flat comparison across an unstructured population.

**Parseval duality and condensation.** Shannon's Parseval identity equates signal energy in the original domain with energy in the spectral domain. On the chirality register the exact Plancherel identity of §5.2 and §13.5 plays that role. Condensation measured by Rényi-2 support M₂ and spectral excitation measured by the WHT energy are the same quantity in dual charts, so concentration and spectrum are not independent climate readouts.

**Restricted alphabets and finite reachability.** Shannon's coding theorems bound achievable rate for a fixed channel law. When the QuBEC input alphabet is restricted, generator-restricted percolation supplies the exact finite counterpart: under fiber-complete restriction the Square-Root Cluster Theorem states that the reachable set from rest satisfies |Reach(A)| = (2^{r(A)})^2, where r(A) is the GF(2) transport rank of the allowed byte set A. Full occupancy of Ω, whose information width is log₂(4096) = 12 bits, requires r(A) = 6 together with odd-parity transport. Restricted alphabets therefore shrink the reachable cluster deterministically as the square of the surviving transport dimension, which is the product geometry of Ω = U × V expressed as a capacity-like limit on what a restricted channel can reach.

**Descent obstructions and the Boolean–Hilbert gap.** Shannon's data-processing discipline is completed, for restricted generators, by algebraic transport obstructions rather than by quotient coarsening alone. Descent of a generator restriction A is classified by the residual module GF(2)^6 / Q(A), where Q(A) is the span of the allowed transport charges. The parity functional is a 1-cocycle that confines even-weight alphabets to even shells, reduces effective rank by one, and bounds the reachable cluster at 1024 states. On horizon ensembles the gap between Boolean sign correlations and Hilbert-lifted inner-product observables equals the real Grothendieck constant of order 2, namely √2. That constant is therefore an exact integrality bound between classical cut-norm readout and Hilbert relaxation on the same site.

The verified computational consequences of this architecture, including hidden-subgroup resolution and exact two-step uniformization, are recorded in Part VII and are not repeated here.
...
Optional: [hQVM_Specs_Formalism.md](hQVM_Specs_Formalism.md), [hQVM_QuBEC_Holography.md](hQVM_QuBEC_Holography.md), [Analysis_hQVM_Percolation.md](docs/references/Analysis_hQVM_Percolation.md), [Analysis_hQVM_Cohomology.md](docs/references/Analysis_hQVM_Cohomology.md), [Analysis Gyroscopic Multiplication](docs/Findings/Analysis_Gyroscopic_Multiplication.md).
