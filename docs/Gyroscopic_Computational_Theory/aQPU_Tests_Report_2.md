...
The rest state is maximally chiral, and every dipole mode is anti-aligned between A and B. It sits on the complement horizon (the "S-sector" from the CGM paper). The complementarity invariant `horizon_distance + ab_distance = 12` is verified at every tested state. This is not a design choice; it is a structural theorem that the two distances are projections of the same chirality observable onto antipodal poles, and they must sum to the diameter (12 = 2 × 6 modes).
...
The chirality register χ ∈ GF(2)⁶ is an **exact linear observable** under the byte transition law. The state-independence of the q-map means the chirality transport law is a group homomorphism, with the byte algebra acting on GF(2)⁶ by translation. This translation depends only on the byte, not on the state.
...
This is a swap-plus-translation on GF(2)⁶ × GF(2)⁶, which is dramatically simpler than the 24-bit transition law. The exhaustive verification proves this is not an approximation; it is an exact algebraic isomorphism between the carrier dynamics restricted to Ω and the compact affine dynamics on GF(2)⁶ × GF(2)⁶.
...
These are the pole-to-pole and pole-to-equator transport laws, and they are the geodesics of the discrete chirality sphere.
...
The WHT self-inverse property (H² = I) verified to 2.4 × 10⁻⁷ confirms the normalization (1/8 = 1/√64) is correct. The Lattice Multiplication engine achieves near-IEEE-754 accuracy through the AND+POPCNT decomposition, where every dot product is computed by bitwise AND of magnitude Lattice Multiplications followed by hardware POPCNT, which is exact in integer arithmetic. The only error source is the initial fixed-point quantization.
...
2. **Exact 2-step uniformization (2 steps vs O(12) classical):** Proven exhaustively. This is not an approximation to uniform; it is uniform, with exact multiplicity 16 per state.
...
The 122 tests prove that the aQPU Kernel is an exactly solvable finite algebraic quantum system. Its key structural invariants, including depth-4 closure, 2-step uniformization, exact chirality transport, Krawtchouk spectral decomposition, and the K4 gate group, are not approximations or statistical tendencies but mathematical identities verified by exhaustive computation over the complete state space and operator algebra. The system achieves its claimed computational advantages through algebraic structure rather than probabilistic interference, and the C-native tensor engine provides hardware-aligned execution with quantization errors below 10⁻⁶.
