...
def calculate_stage_thresholds() -> Dict[str, float]:
    ...
    return {"CS": s_p, "UNA": u_p, "ONA": o_p, "BU": m_a}
...
def calculate_energies(
    actions: Dict[str, float], s_gut: float, scale_A: float = 1.0
) -> Dict[str, float]:
    """
...
def anchor_by(
    stage: str, e_value: float, actions: Dict[str, float], s_gut: float
...
def bu_dual_project(
    uv_energies: Dict[str, float], e_ew: float = 246.22
) -> Dict[str, float]:
    """
...
def bu_dual_project_geometry(
    actions: Dict[str, float], e_ew: float = 246.22
) -> Dict[str, float]:
    """
...
def calculate_optical_invariant(
    uv_energies: Dict[str, float], ir_energies: Dict[str, float]
) -> Dict[str, float]:
    """
...
def calculate_magnification_swap(
    uv_energies: Dict[str, float], ir_energies: Dict[str, float]
) -> Dict[str, float]:
    """
    ...
    if "UV_ONA/UNA" in swap_ratios and "IR_UNA/ONA" in swap_ratios:
        swap_ratios["swap_verified"] = (
            abs(swap_ratios["UV_ONA/UNA"] - swap_ratios["IR_UNA/ONA"]) < 1e-10
        )
...
def main():
    ...
    print(
        f"   Expected invariant: (E_CS x E_EW)/(4*pi^2) = {expected_invariant:.2e} GeV^2"
    )
    ...
    print(
        f"   Angle theta = arctan(S_ONA/S_UNA) = {theta_rad:.6f} rad = {theta_deg:.1f} degrees"
    )
    ...
    for stage in ["CS", "UNA", "ONA", "BU"]:
        if stage in ir_to_uv:
            ...
            print(
                f"   E_{stage:4s}: {original:.2e} -> {recovered:.2e} (error: {error:.2e})"
            )
    ...
    for stage in ["CS", "UNA", "ONA", "BU"]:
        if stage in uv_energies and stage in ir_energies:
            ...
            print(
                f"   alpha_g^{stage:4s}: UV={alpha_g_uv:.2e}, IR={alpha_g_ir:.2e}, suppression={suppression:.2e}"
            )
    ...
    for eta in eta_values:
        ...
        print(
            f"   eta={eta:3.1f}: S_GUT={s_gut_eta:.6f}, E_GUT^UV={e_gut_uv_eta:.2e} GeV, E_GUT^IR={e_gut_ir_eta:.2f} GeV"
        )
    ...
    print(
        "   - Solid-angle dilution (4*pi)^(-2) explains why gravity appears weak in IR"
    )
...
def report_qg_influence(actions, uv_energies, ir_energies):
    ...
    print(
        f"E_BU/E_CS (from Q_G) = {1.0/piQG:.6f}  |  reported = {uv_energies['BU']/uv_energies['CS']:.6f}"
    )
    print(
        f"E_CS^IR  (from Q_G) = {246.22/piQG:.2f} GeV  |  reported = {ir_energies['CS']:.2f} GeV"
    )
    const = uv_energies["CS"] * 246.22 / piQG
    print(f"Invariant K = (E_CS*E_EW)/(pi*Q_G) = {const:.2e} GeV^2")
    print(
        "Q_G appears as the global 'dilution' factor 1/(pi*Q_G) = 1/(4*pi^2) throughout the framework"
    )


# === Gauge coupling running (1-loop, SM) and unification check ===
def run_gauge_couplings_SM(
    mz=91.1876, alpha_em_inv=127.955, sin2thw=0.23126, alpha3=0.1184, mu_list=None
):
    """
...
def report_unification_check(results_from_main):
    ...
    print(
        f"Your UV GUT scale (from actions): E_GUT^UV ~ {egut:.2e} GeV; E_CS ~ {ecs:.2e} GeV"
    )
    if mu12 is not None:
        ratio = egut / mu12
        ...
        if ratio > 1:
            print(
                "Your GUT sits above the SM alpha1=alpha2 crossing (common in non-SUSY)."
            )
        else:
            print("Your GUT sits at or below the SM alpha1=alpha2 crossing.")
    print(
        "Note: If you want exact unification, either thresholds or extra fields (SUSY-like) are typically needed."
    )
...
def report_proton_decay_bound(results_from_main):
    ...
    if mu12 is None:
        print(
            "Could not find alpha1=alpha2 crossing below the scan ceiling; using alpha_GUT^{-1} ~ 40 as a placeholder."
        )
        alpha_GUT_inv = 40.0
    else:
        alpha_GUT_inv = a12
    ...
    print(
        "CGM geometry determines E_GUT^UV exactly; proton lifetime follows from this geometric scale."
    )
...
def report_breaking_chain():
    ...
    print("Chain:")
    print(
        "  SU(3)_c x SU(2)_L x SU(2)_R x U(1)_chi  --(v_R~M_R)-->  SU(3)_c x SU(2)_L x U(1)_Y"
    )
    print(
        "                                                   --(v=246.22 GeV)-->  SU(3)_c x U(1)_em"
    )
    print("Embeddings:  Y = T3_R + (B-L)/2 ,   Q = T3_L + Y")
    print(
        "Interpretation:  UNA ~ left rotations, ONA ~ right rotations/translations, BU ~ U(1) memory."
    )


def report_neutrino_fit_and_mixing(
    results_from_main, hierarchy="NH", mlight_eV=0.0, include_half=False
):
    """
    ...
    M_R = results_from_main["uv_energies"]["GUT"] / (
        48.0**2
    )  # your quantised heavy scale
    ...
    print(
        f"M_R (from GUT/48^2) = {M_R:.2e} GeV  |  convention factor = {fac} (1 means your current)"
    )
    ...
    print(
        f"Active-sterile mixing angles: theta1={th1:.3e}, theta2={th2:.3e}, theta3={th3:.3e}"
    )
    print(
        "Tiny theta_i confirms the heavy state is effectively sterile (CS stays hidden)."
    )
...
def report_WR_Zprime_masses(results_from_main, gR=0.65, gBL=0.40):
    ...
    v_R = results_from_main["uv_energies"]["GUT"] / (
        48.0**2
    )  # identify v_R with M_R scale you already printed
    M_WR = 0.5 * gR * v_R
    ...
    print(
        "These are far above any collider reach, consistent with no direct observation of the CS/sterile sector."
    )
...
if __name__ == "__main__":
    ...
    report_qg_influence(
        results["actions"], results["uv_energies"], results["ir_energies"]
    )
    ...
    report_neutrino_fit_and_mixing(
        results, hierarchy="NH", mlight_eV=0.0, include_half=False
    )
