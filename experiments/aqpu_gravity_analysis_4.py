...
def csm_mu_gravity_bridge():
    ...
    print()
    print("  Total closure credit rho_MU * E_ref^2 is preserved across all psi.")
    print()
...
def nonlinear_system_summary():
    ...
    print("  g = -grad Phi")
    print("  div g = -Q_G * G(x) * rho_A(x)")
    print("  curl g = -dB_g/dtau")
    print()
    print("Gravitomagnetic sector:")
    print("  div B_g = 0")
    print("  curl B_g = -(Q_G * G(x)/c^2) * J_A + (1/c^2) * dg/dtau")
    print()
    print("Source conservation:")
    print("  d rho_A/dtau + div J_A = 0")
    print("  d J_A/dtau + div sigma_A = rho_A * g + (1/c^2) * (J_A x B_g)")
    print()
    print("Stress closure (kernel-fixed):")
    print("  sigma_A = p_A * I + pi_A  (1 + 5 decomposition)")
    print("  ||pi_A||^2 / Tr(sigma_A)^2 = 2/75  (bulk shell invariant)")
    print()
...
def verify_einstein_tensor_decomposition() -> None:
    ...
    print("  G_tt / r^2 = 2(dpsi/dr)/r^2 = 8*pi*G(psi)*rho_A  (c=1)")
    print("  div g = -4*pi*G(psi)*rho_A")
    print("  div[(G_0/G(psi))*g] = -4*pi*G_0*rho_A = -Q_G*G_0*rho_A")
...
def section_E_ref_formal_proof() -> None:
    ...
    print()
    print("Premise 3: STF-verified optical depth")
