...
def delta_self_consistency_rhs(
    delta: float, *, include_third_order: bool = True
) -> float:
    """
...
def solve_delta_self_consistency(
    delta_guess: float = DELTA, *, include_third_order: bool = True, max_iter: int = 30
) -> tuple[float, float]:
    """
...
def shell_path_a_ladder() -> tuple[ShellPathAStep, ...]:
    ...
    return (
        ShellPathAStep(
            "Top", "id", "|H| + C2 - C1", HORIZON_CARDINALITY + CODE_C2 - CODE_C1
        ),
        ShellPathAStep("Higgs", "W2", "M_shell/2", M_SHELL // 2),
        ShellPathAStep(
            "Z", "W2'", "M_shell/2 + C1 + C2", M_SHELL // 2 + CODE_C1 + CODE_C2
        ),
...
def eval_law_terms(
    ch: ChannelCoeffs, delta: float, *, order: int = 5
) -> LawTermBreakdown:
...
def ew_omega_path_trace(
    label: str, micro_ref: int = 63, delta: float = DELTA
) -> EwOmegaPathTrace:
    ...
    arch = tuple(
        (
            int(r["arch_shell"])
            for r in g.trace_word_steps(word, micro_ref=micro_ref)[1:]
        )
    )
...
def ew_omega_path_traces_all(
    micro_ref: int = 63, delta: float = DELTA
) -> tuple[EwOmegaPathTrace, ...]:
    return tuple(
        (
            ew_omega_path_trace(lbl, micro_ref, delta)
...
def verify_omega_path_ladder(delta: float = DELTA) -> dict[str, bool | float]:
    ...
    traces = {
        t.label: t
        for t in (
            ew_omega_path_trace(lbl, 63, delta) for lbl in ("Top", "Higgs", "Z", "W")
        )
    }
    ok_poly = all(
        abs(
            carrier_trace_polynomial_breakdown(ch.label, delta).total
            - eval_law(ch, delta)
        )
        < 1e-12
        for ch in CHANNELS
    ...
    for d in ew_carrier_path_decomposition(observed, delta, v, order=order):
        ...
        rows.append(
            EwMassAnchor(
                ...
                mass_obs_gev=observed[d.observable],
                mass_pred_gev=m_pred,
                l_obs=d.l_obs,
                l_pred=d.l_pred,
                n_obs=d.shell_dist,
                n_pred=d.l_pred / delta,
                shell_residual_ticks=d.residual_ticks,
                mass_ppm=mass_ppm_err(m_pred, observed[d.observable]),
            )
        )
    return tuple(rows)


def verify_ew_mass_identification(
    observed: dict[str, float], delta: float = DELTA, v: float | None = None
) -> dict[str, bool | float | tuple[EwMassAnchor, ...]]:
    """
    ...
    m_obs = observed
    rc = ew_couplings_from_masses(
        m_obs["Top quark mass energy"],
        m_obs["Higgs mass energy"],
        m_obs["Z boson mass energy"],
        m_obs["W boson mass energy"],
        v,
    )
    ...
    alpha_rel = (
        mc.alpha_ew_inv / rc.alpha_ew_inv - 1.0 if rc.alpha_ew_inv else float("nan")
    )
    return {
        "rows": rows,
        "sub_ppm_top": ppm_top < 1.0,
        "sub_ppm_wz": ppm_wz < 1.0,
        "ppm_top": ppm_top,
        "ppm_higgs": ppm_higgs,
        "ppm_z": ppm_z,
        "ppm_w": ppm_w,
        "alpha_geometric": ALPHA_GEOMETRIC,
        "alpha_inv_pred": mc.alpha_ew_inv,
        "alpha_inv_pdg": rc.alpha_ew_inv,
        "alpha_rel_err": alpha_rel,
        "alpha_digits_match": abs(alpha_rel) < 1e-06,
        "max_shell_ticks": max((abs(r.shell_residual_ticks) for r in rows)),
        "shell_ticks_ok": all((abs(r.shell_residual_ticks) < 0.2 for r in rows)),
    }
...
def model_masses_from_path(
    ...
    return {
        "m_top": predicted_mass_from_path("Top", delta, v),
        "m_higgs": predicted_mass_from_path("Higgs", delta, v),
        "m_z": predicted_mass_from_path("Z", delta, v),
        "m_w": predicted_mass_from_path("W", delta, v),
    }
...
def verify_ew_ladder_derivations() -> dict[str, bool | float]:
    ...
    return {
        ...
        "r5_top_matches_wz_gap": abs(
            channel_r5_from_code_curvature("Top") - r5_constant_from_wz_gap()
        )
        < 1e-12,
...
def electroweak_assignment_uniqueness_audit(
    ...
    for f_top in range(8):
        ...
        p_t, q_t, r5_t = (
            _pq_from_flags(a_t, r_t, b_t)[0],
            _pq_from_flags(a_t, r_t, b_t)[1],
            _r5_from_flags(a_t, r_t, b_t),
        )
        for f_h in range(8):
            a_h, r_h, b_h = (bool((f_h >> 2) & 1), bool((f_h >> 1) & 1), bool(f_h & 1))
            p_h, q_h, r5_h = (
                _pq_from_flags(a_h, r_h, b_h)[0],
                _pq_from_flags(a_h, r_h, b_h)[1],
                _r5_from_flags(a_h, r_h, b_h),
            )
            for f_z in range(8):
                a_z, r_z, b_z = (
                    bool((f_z >> 2) & 1),
                ...
                p_z, q_z, r5_z = (
                    _pq_from_flags(a_z, r_z, b_z)[0],
                    _pq_from_flags(a_z, r_z, b_z)[1],
                    _r5_from_flags(a_z, r_z, b_z),
                )
                for f_w in range(8):
                    a_w, r_w, b_w = (
                        bool((f_w >> 2) & 1),
                    ...
                    p_w, q_w, r5_w = (
                        _pq_from_flags(a_w, r_w, b_w)[0],
                        _pq_from_flags(a_w, r_w, b_w)[1],
                        _r5_from_flags(a_w, r_w, b_w),
                    )
                    ...
                    for label, obs_n, (a_ch, b_ch, c_ch), p_val, q_val, r5_val in [
                        ...
                        l_pred = eval_law(
                            ChannelCoeffs(
                                label, "", a_ch, b_ch, c_ch, p_val, q_val, r5_val
                            ),
                            delta,
                        )
    ...
    trace_rows.sort()
    declared_rank = (
        trace_rows.index(declared_err) + 1 if declared_err is not None else -1
    )
...
def ew_carrier_path_decomposition(
    observed: dict[str, float],
    delta: float = DELTA,
    v: float | None = None,
    *,
    order: int = 5,
) -> tuple[EwCarrierPathDecomposition, ...]:
    """
    ...
    for ch in CHANNELS:
        ...
        rows.append(
            EwCarrierPathDecomposition(
                ...
                terms=terms,
                l_pred=l_pred,
                residual_ticks=(l_obs - l_pred) / delta,
                residual_d6=(l_obs - l_pred) / delta**6,
            )
        )
    return tuple(rows)
...
def _solve_wz_phase_corrected(
    l_wz: float, *, guess: float = 0.0207, iters: int = 20
) -> float:
    """
...
def electroweak_backsolves(observed: dict[str, float]) -> tuple[DeltaBacksolve, ...]:
    ...
    return (
        ...
        DeltaBacksolve(
            "W/Z", "D[9-10D+2D^2/sqrt(5)-D^3]", _solve_wz_phase_corrected(l_wz)
        ),
    )
...
def shell_transition_matrix(q: int) -> tuple[tuple[Fraction, ...], ...]:
    return tuple(
        (tuple((shell_transition_prob(w, q, wp) for wp in range(7))) for w in range(7))
    )
...
def lepton_delta_coords(n_h_law: float, delta: float = DELTA) -> dict[str, float]:
    ...
    return {
        ...
        "n_electron": lepton_base_n(14, "electron")
        + SU2_RESIDUAL
        + delta / (n_h_law * delta / KERNEL_BYTE_APERTURE),
    }
...
def lepton_ladder_residuals(
    observed: dict[str, float], delta: float = DELTA, v: float = E_EW_GEV
    ...
    specs = [
        ...
        (
            ...
            0.0,
            Fraction(-51, 256)
            * float(CARRIER_TRACES[2] - CARRIER_TRACES[4])
            * delta**3,
            "+SU2+(5/256)/n_H",
            "",
            "+(-51/256)*(C2-C4)*D^3",
...
def lepton_d3_transition_costs() -> tuple[LeptonD3TransitionCost, ...]:
    wz = CODE_C2 - CODE_C1
    specs = (
        ("tau", 5, 4, Fraction(-27, 64), "-3*(C2-C1)", Fraction(-3 * wz, 1)),
        (
            "muon",
            4,
            2,
            Fraction(-37, 64),
            "-(|H|-3*(C2-C1))",
            Fraction(-(HORIZON_CARDINALITY - 3 * wz), 1),
        ),
        (
            "electron",
            4,
            ...
            Fraction(-51, 256),
            "-(3*|K4|+C1/8)",
            -(Fraction(12, 1) + Fraction(CODE_C1, 8)),
        ),
    )
...
def lepton_d3_selection_rule_probe() -> tuple[LeptonD3SelectionRule, ...]:
    ...
    return (
        LeptonD3SelectionRule("tau", "-(C1/2)*(C2-C1)", tau_cost, Fraction(-27, 1)),
        LeptonD3SelectionRule(
            "muon", "-(|H|-|(tau 64-cost)|)", muon_cost, Fraction(-37, 1)
        ),
        LeptonD3SelectionRule(
            "electron", "-(3*|K4|+C1/8)", electron_cost, Fraction(-51, 4)
        ),
    )
...
def lepton_d3_path_breaking_probe() -> LeptonD3PathBreakingProbe:
    ...
    tau = _k4_phase_selector(
        "tau memory", True, False, False, Fraction(-27, 1), "-(C1/2)*(C2-C1)"
    )
    muon = _k4_phase_selector(
        "muon hard path", True, True, False, Fraction(-37, 1), "-(|H|-|tau|)"
    )
    electron = _k4_phase_selector(
        "electron reset path", False, False, True, Fraction(-51, 4), "-(3*|K4|+C1/8)"
    )
    ...
    conservation_electron_dyadic = (
        -(tau_coeff + muon_coeff) / carrier_delta_muon_electron
    )
    ...
    return {
        ...
        "implemented_muon_electron_split": muon.normalized_64_cost
        - electron.normalized_64_cost,
        "implemented_muon_electron_split_dyadic": Fraction(-37, 64)
        - Fraction(-51, 256),
        "simplified_electron_64_cost": simplified_electron_64,
        "simplified_muon_electron_split": muon.normalized_64_cost
        - simplified_electron_64,
        "simplified_muon_electron_split_dyadic": Fraction(-37, 64) - Fraction(-3, 16),
        "claim_25_over_64_matches_implemented": Fraction(-37, 64) - Fraction(-51, 256)
        == Fraction(-25, 64),
        "claim_25_over_64_matches_simplified": Fraction(-37, 64) - Fraction(-3, 16)
        == Fraction(-25, 64),
        "electron_reset_offset_from_simplified": Fraction(-51, 256) - Fraction(-3, 16),
        ...
        "electron_from_history_matches": electron_from_history_64
        == electron.normalized_64_cost,
        ...
        "carrier_conservation_current_offset": Fraction(-51, 256)
        - conservation_electron_dyadic,
        "carrier_conservation_current_matches": Fraction(-51, 256)
        == conservation_electron_dyadic,
    }
    ...
    for q_a, q_b in ((2, 4), (5, 4)):
        ...
        rows.append(
            QSupportDuality(
                ...
                complement_pair_hits=sum((1 for word in a if word ^ 63 in b)),
            )
        )
    return tuple(rows)
    ...
    for byte, gate in gates:
        ...
        rows.append(
            HorizonGateSelection(
                ...
                selected_by_electron_reset=is_s_gate
                and zero_intron
                and zero_payload
                and zero_family,
            )
...
def source_traceability_probe() -> SourceTraceabilityProbe:
    ...
    return SourceTraceabilityProbe(
        ...
        selected_is_gene_mic=len(selected) == 1 and selected[0].byte == 170,
...
def lepton_horizon_wrap_exhaustion_probe(
    ...
    tau_mu_budget = abs(costs["tau"].normalized_64_cost) + abs(
        costs["muon"].normalized_64_cost
    )
...
def shell_spectral_weights() -> tuple[ShellSpectralWeight, ...]:
    ...
    for q in range(7):
        M = shell_transition_matrix(q)
        shell_frob = sum(
            (M[i][j] * M[i][j] for i in range(7) for j in range(7)), Fraction(0)
        )
        rows.append(
            ShellSpectralWeight(
                q=q,
...
def strong_scale_bulk_probe(
    lambda_qcd_gev: float = 0.2, delta: float = DELTA, ew_scale_gev: float = E_EW_GEV
...
def ew_couplings_from_masses(
    m_t: float, m_h: float, m_z: float, m_w: float, v: float = E_EW_GEV
) -> EWCouplings:
...
def hzw_leave_one_out(
    observed: dict[str, float], backsolves: tuple[DeltaBacksolve, ...]
