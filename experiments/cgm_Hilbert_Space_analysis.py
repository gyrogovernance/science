...
4. Verification of the foundational conditions (CS, UNA, ONA, BU-Egress, BU-Ingress) as operator relations
...
# Choose rotation by pi about x-axis (order 2, ensures depth-four order)
...
print("Joint spectrum conditions from Proposition BU-Egress:")
...
print(
    "    - ω encodes the foundational assumption and lemmas (CS, UNA, ONA, BU, Memory) as expectation conditions"
)
print()
...
try:
    ...
    # For BU-Egress: Δ = 0 for all small t ⇒ nested commutator conditions
    print("BCH analysis for BU-Egress:")
    print(
        "  Delta = 2 t^2 [X,Y] + O(t^4); the t^3 terms cancel in the LRLR-RLRL difference."
    )
    print(
        "  The su(2)-type nested-commutator conditions ([X,[X,Y]] = aY, [Y,[X,Y]] = -aX)"
    )
    print(
    ...
    print("CONCLUSION (symbolic): Proposition BU-Egress + higher-order BCH conditions")
    print("  + simplicity + compactness uniquely select su(2).")

except ImportError:
    print("SymPy unavailable; using theoretical BCH result.")
    print("Delta = 2 t^2 [X,Y] + O(t^4); t^3 terms cancel. The su(2)-type conditions")
...
if memory_holds_mem:
    ...
    print("  Prior conditions reconstructed in balanced state")
else:
...
if comm_norm_S < 1e-6:
    ...
    print("  The t^2 term vanishes in the S-sector, leaving only O(t^3) conditions")
...
print("Summary vs foundational conditions (Hilbert-side, S-sector):")
...
if True:  # Always run S1 flow tests
    ...
    if not holds_uniform:
        ...
        print("  Therefore: 2D fails under RA+BCH conditions that select su(2) in 3D.")
    else:
        print(
