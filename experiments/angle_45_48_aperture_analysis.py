...
class Angle45_48ApertureAnalyzer:
    ...
    def analyze_aperture_transition(self) -> Dict:
        ...
        return {
            ...
            "fractional_relationships": {
                ...
                "fraction_48_over_45": Fraction(
                    int(round(self.angle_48)), int(round(self.angle_45))
                ),
                "fraction_45_over_48": Fraction(
                    int(round(self.angle_45)), int(round(self.angle_48))
                ),
                "exact_ratio": "16/15",
            },
            "musical_analysis": {
    ...
    def generate_comprehensive_analysis(self) -> str:
        ...
        output.append(
            f"Difference = {ang['difference_deg']}° = {ang['difference_rad']:.6f} rad"
        )
        ...
        output.append(
            f"45° closure = {closure['closure_45_percent']:.2f}% (perfect square)"
        )
        output.append(
            f"48° closure = {closure['closure_48_percent']:.2f}% (aperture square)"
        )
        ...
        output.append(
            f"Musical interval = {aperture_data['musical_analysis']['interval_name']}"
        )
        ...
        output.append(
            f"Aperture ratio (CGM/45°→48°) = {cgm['aperture_ratio_cgm_to_angle']:.3f}"
        )
        output.append("")
        ...
        output.append(
            f"Both represent closure with aperture: {theory['both_represent_closure_with_aperture']}"
        )
    ...
    def run_analysis(self):
        ...
        # Save results
        with open(
            "experiments/results_45_48_aperture_analysis.txt", "w", encoding="utf-8"
        ) as f:
            f.write(analysis)
