...
def infer_bridge_exponent_from_H0(H0_SI, const, scales):
    ...
    return {
        'n_emp_aperture': n_emp_ap,
        'n_emp_no_aperture': n_emp_noap,
        'F_obs': F_obs
    }
...
def print_analysis():
    ...
    print("-" * 80)
    print(f"Observed dark energy density:   ρ_Λ,obs = {float(cc_terms['rho_L_obs']):.6e} J/m³")
    print(f"Vacuum energy estimate:         ρ_vac   = {float(cc_terms['rho_vac']):.6e} J/m³")
    ...
    print("-" * 80)
    print(f"Λ_obs (from 3H₀²/c²):           {float(lambda_terms['Lambda_obs']):.6e} m⁻²")
    print()
    ...
    print(f"Critical density:               ρ_crit = {float(eos['rho_crit']):.6e} kg/m³")
    ...
    uncert = propagate_uncertainties(cosmo.H0_planck, cosmo.H0_planck_error, const, cosmo)
    print("UNCERTAINTY PROPAGATION")
    print("-" * 80)
    print(f"B_i = ({float(uncert['B_i']):.6e} ± {float(uncert['B_i_err']):.2e}) m²⋅K⋅kg⁻²")
    print(f"  Relative error: {float(uncert['rel_Bi']):.6e}")
    print()
    print(f"ρ_Λ = ({float(uncert['rho_L']):.6e} ± {float(uncert['rho_L_err']):.2e}) kg/m³")
    ...
    print()
    print(f"  L_H / L_Pl ratio:             10^{float((scales['L_ratio_H_to_Pl']).ln()/Decimal('10').ln()):.1f}")
    ...
    print(f"  T_CMB / T_eq ratio:           10^{float((scales['T_ratio_CMB_to_eq']).ln()/Decimal('10').ln()):.1f}")
    ...
    print(f"  ρ_Pl / ρ_H ratio:             10^{float((scales['rho_ratio_Pl_to_H']).ln()/Decimal('10').ln()):.1f}")
    print()
    ...
    print(f"  Without aperture:             n = {float(n_emp['n_emp_no_aperture']):.6f}")
