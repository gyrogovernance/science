...
def check_iteration_scaling(
    ...
    return {
        ...
        "amplitude_valid": (
            mp.almosteq(amp_ratio, mp.mpf("0.25"), rel_eps=mp.mpf("5e-3"))
            if amp_ratio != mp.mpf("inf")
            else False
        ),
    }
...
def anchor_uv_energies(
    actions: Dict[str, Any], S_gut: Any, E_CS_GeV: float
) -> Dict[str, float]:
...
def bu_dual_project(uv: Dict[str, float], E_EW_GeV: float = 246.22) -> Dict[str, float]:
    """
...
def optical_invariant_values(
    uv: Dict[str, float], ir: Dict[str, float]
) -> Dict[str, float]:
...
def magnification_swap(uv: Dict[str, float], ir: Dict[str, float]) -> Dict[str, Any]:
    out: Dict[str, Any] = {}
    if "UNA" in uv and "ONA" in uv:
        out["UV_ONA/UNA"] = uv["ONA"] / uv["UNA"]
    if "UNA" in ir and "ONA" in ir:
        out["IR_UNA/ONA"] = ir["UNA"] / ir["ONA"]
...
def print_summary(
    ...
    print(
        f"   π ≈ {mp.nstr(iterations[-1].mean_value, 15)} ; rel err = {mp.nstr(iterations[-1].relative_precision, 2)}"
    )
    if len(iterations) >= 2:
        sca = check_iteration_scaling(iterations[-2], iterations[-1])
        print(
            f"   gap ratio ~ {mp.nstr(sca['gap_ratio'],6)} (→0.25), amplitude ratio ~ {mp.nstr(sca['amplitude_ratio'],6)} (→0.25)"
        )
    ...
    print(
        f"   S_min=(π/2) m_a = {mp.nstr(core.S_min,10)} ; S_rec=(3π/2) m_a = {mp.nstr(core.S_rec,10)} ; S_geo=  m_a π√3/2 = {mp.nstr(core.S_geo,10)}"
    )
    ...
    print(
        "\n5) IR energies via optical conjugacy (E^UV E^IR = (E_CS·246.22 GeV)/(4π²))"
    )
    for k in ("CS", "UNA", "ONA", "BU", "GUT"):
        print(f"   E_{k:<3}^IR = {E_ir[k]:.6e} GeV")
    inv = optical_invariant_values(E_uv, E_ir)
    expected = (E_uv["CS"] * 246.22) / (float(4 * mp.pi * mp.pi))
    print(f"\n   invariant K = (E_CS·246.22)/(4π²) = {expected:.6e} GeV²")
    ok = True
    ...
    if fs:
        print(
            f"   δ_BU = {fs['delta_BU']:.9f} ;  m_a = {fs['m_a']:.9f} ; α_pred = {fs['alpha_pred']:.10f}"
        )
    else:
...
def main(
    ...
    # Phase 5: IR via optical conjugacy (EW = 246.22 GeV)
    E_ir = bu_dual_project(E_uv, E_EW_GeV=246.22)
