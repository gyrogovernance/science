...
def compute_delta_BU_from_gyrovector():
    ...
    assert abs(delta_BU_mp - expected_delta_BU) < tolerance, \
...
def print_metadata():
    ...
    print("Sources: Integrated from cgm_3D_6DoF (gyrotriangle), tw_closure_test (gyrovector).")
...
def main():
    ...
    print(f"delta_BU = {float(delta_BU):.12f} rad (computed from gyrovector dual-pole monodromy)")
    ...
    print(f"Error: {float(error_base_ppm):.3f} ppm (319 ppm accuracy from geometry alone)")
    ...
    for ndec in rounding_levels:
        ...
        print(f"  delta_BU rounded to {ndec} dec: {rounded_delta} -> alpha_0 = {float(alpha_rounded):.9f} (err = {float(err_rounded_ppm):.1f} ppm)")
    print("  Observation: Base α₀ requires δ_BU to at least 5–6 decimals for ppm-level accuracy;")
    ...
    print("This represents 3 significant figures of agreement with no fitted parameters.")
