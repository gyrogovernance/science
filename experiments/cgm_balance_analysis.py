...
def analyze_balance_bands(H_curv_SI, hbar, Xi_EW=None):
    ...
    results = {"cgm_scales": {}, "particles": {}, "bands": []}
...
def compute_natural_breaks(particles_dict, k=3, use_delta=False, Xi_EW=None):
    ...
    if use_delta and Xi_EW is not None:
        items = sorted(
            [
                (name, data["log10_Xi"] - math.log10(float(Xi_EW)))
        ...
        items = sorted(
            [(name, data["log10_Xi"]) for name, data in particles_dict.items()],
            key=lambda x: x[1],
        )
...
def print_results():
    ...
    datasets = [
        (
            ...
            cosmo.H0_planck,
            cosmo.H0_planck_error,
        ),
        ("SH0ES 2022 (Cepheid-SNe Ia)", cosmo.H0_shoes, cosmo.H0_shoes_error),
    ]
    ...
    for name, H0, H0_err in datasets:
        ...
        print(
            f"  B_i = ({results['Sigma_eq']:.6e} ± {results['Sigma_eq_error']:.2e}) m²⋅K⋅kg⁻²"
        )
        ...
        assert (
            abs(results["verification_ratio"] - 1.0) < 1e-15
        ), "Verification ratio must equal 1"
    ...
    chi = (
        const.k_B * planck_results["Sigma_eq_decimal"] * planck_results["H0_SI_decimal"]
    ) / (const.G * const.c)
    ...
    I_top = (
        const.k_B * planck_results["T_eq_decimal"] * planck_results["A_H_decimal"]
    ) / (
        const.hbar
        * planck_results["H0_SI_decimal"]
        * Decimal("2")
        * planck_results["R_H_decimal"] ** 2
    )
    ...
    assert (
        abs(check_QG_aperture - Decimal("0.5")) < 1e-15
    ...
    print(
        f"  ΔG/G:                         {rel_err_G:.6e} ({100*rel_err_G**2/rel_err_total**2:.1f}%)"
    )
    print(
        f"  ΔH₀/H₀:                       {rel_err_H0:.6e} ({100*rel_err_H0**2/rel_err_total**2:.1f}%)"
    )
    print(f"  Total ΔΣ/Σ:                   {rel_err_total:.6e}")
    print(
        f"  → B_i precision limited by H₀ measurement ({100*rel_err_H0**2/rel_err_total**2:.0f}%)"
    )
    print()
    ...
    I1_CGM_computed = (
        const.k_B
        * planck_results["Sigma_eq_CGM_decimal"]
        * (planck_results["H0_SI_decimal"] / (Decimal("1") + cgm.m_a))
    ) / (const.G * const.c)
    ...
    print(
        f"  → Aperture value {'locked by equilibrium ✓' if m_a_deviation < 1e-10 else 'not fully determined'}"
    )
    print()
    ...
    # H0 recovery
    H0_from_BI = (Decimal("2") * const.G * const.c) / (
        const.k_B * planck_results["Sigma_eq_decimal"]
    )
    H0_ratio = H0_from_BI / planck_results["H0_SI_decimal"]
    print(f"H₀ from B_i:                    {float(H0_from_BI):.6e} s⁻¹")
    print(
        f"H₀ input:                       {float(planck_results['H0_SI_decimal']):.6e} s⁻¹"
    )
    ...
    kB_recovered = (Decimal("2") * const.G * const.c) / (
        planck_results["Sigma_eq_decimal"] * planck_results["H0_SI_decimal"]
    )
    ...
    print("Interpretation:")
    print(
        "  • I₁(trans) = 2.000... for all methods: Closure invariant preserved under transport"
    )
    ...
    inv_cgm = calculate_emergent_constancy_invariants(
        Sigma_eq_CGM_dec, H_aperture, const
    )
    ...
    assert abs(Decimal(str(inv_cgm["I1"])) - expected_I1_CGM_dec) < Decimal(
        "1e-10"
    ), "Aperture identity must match"
    print()
    ...
    print()
    print(
        f"Normalized to Planck area (l_P²):      {bridge['ratio_to_planck_area']:.6e}"
    )
    ...
    for method_name, H_curv_method in [
        ("baseline", H_baseline),
        ...
        print(
            f"Particle log₁₀(Ξ) range: {bands['log_range'][0]:.1f} to {bands['log_range'][1]:.1f}"
        )
        ...
        nat_bands, cuts, gaps = compute_natural_breaks(
            bands["particles"], k=3, use_delta=use_delta, Xi_EW=Xi_EW
        )
        ...
        for i, band in enumerate(nat_bands, 1):
            ...
            if i < len(nat_bands):
                print(
                    f"  ──── Break at {label} = {cuts[i-1]:.2f} (gap: {gaps[i-1]:.2f}) ────"
                )
        ...
        if len(ratios) >= 2:
            ...
            print(
                f"    Band 1→2: ≈ 10^{math.log10(ratios[0]):.1f} (neutrino seesaw regime)"
            )
            print(
                f"    Band 2→3: ≈ 10^{math.log10(ratios[1]):.1f} (generational scaling)"
            )
        print()
    ...
    print()
    print(
        f"Balance Index (Planck):          B_i = {planck_results['Sigma_eq']:.6e} m²⋅K⋅kg⁻²"
    )
    print(
        f"Balance Index (SH0ES):           B_i = {shoes_results['Sigma_eq']:.6e} m²⋅K⋅kg⁻²"
    )
    ...
    baseline_bands, baseline_cuts, baseline_gaps = compute_natural_breaks(
        bands_baseline["particles"], k=3
    )
