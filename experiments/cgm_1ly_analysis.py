...
def compute_phase_time_depth(
    d_AU, cycle_time=year_tropical, tidal_exponent=1.0, T_coh=None
):
    """
...
def compute_fourier_spectrum(max_distance_ly=5.0, samples=512, tidal_exponent=1.0):
    ...
    return [
        {"frequency_cycles_per_ly": freqs[i], "magnitude": magnitudes[i]}
...
def main():
    ...
    print(
        f"Aperture fraction:             {APERTURE_TRANSMISSION:.4f} = {APERTURE_TRANSMISSION*100:.2f}%"
    )
    ...
    print(
        f"Using tidal exponent p = {tidal_exp:.3f}. (Set TIDAL_EXPONENT to adjust this assumption.)"
    )
    print()
    ...
    print(
        f"{'Threshold':<25} {'k-factor':<12} {'Distance (AU)':<15} {'Distance (ly)':<15} {'Phase/pi':<10}"
    )
    for name, data in boundaries.items():
        print(
            f"{name:<25} {data['k_factor']:<12.6f} {data['distance_AU']:<15.0f} {data['distance_ly']:<15.4f} {data['phase_accumulated']/math.pi:<10.3f}"
        )
    print()

    print("5. OORT CLOUD STRUCTURE WITH CONFIDENCE BANDS")
    print(
        f"{'Oort Region':<30} {'Distance (AU)':<25} {'Distance (ly)':<15} {'Phase/pi':<10} {'Multiplicity':<12} {'Left Bias (%)':<13}"
    )
    for region, (distance_AU, uncertainty_AU) in oort_boundaries.items():
        ...
        dist_range = (
            f"{distance_AU - uncertainty_AU:,} - {distance_AU + uncertainty_AU:,}"
        )
        left_bias_pct = effects["parity_ratio"] * 100
        print(
            f"{region:<30} {dist_range:<25} {distance_ly:<15.4f} {effects['phase_fraction_of_pi']:<10.3f} {effects['multiplicity']:<12} {left_bias_pct:<12.2f}%"
        )
    print()
    ...
    print(
        f"{'CGM Threshold':<25} {'Predicted (AU)':<15} {'Oort Feature':<25} {'Observed (AU)':<25}"
    )
    for cgm_name, cgm_au, oort_name, oort_au, oort_unc in matches:
        ...
        print(
            f"{cgm_name:<25} {cgm_au:<15,} {oort_name:<25} {oort_str:<25} ({deviation})"
        )
    print()
    ...
    print(
        f"{'Distance (AU)':<15} {'Phase/pi':<10} {'Coherence':<12} {'Multiplicity':<12} {'Left Bias (%)':<13}"
    )
    for d in oort_test_distances:
        effects = aperture_effects(d, year_tropical, tidal_exp)
        left_bias_pct = effects["parity_ratio"] * 100
        print(
            f"{d:<15,} {effects['phase_fraction_of_pi']:<10.3f} {effects['coherence']:<12.4f} {effects['multiplicity']:<12} {left_bias_pct:<12.2f}"
        )
    print()
    ...
    print(
        f"{'Distance (AU)':<15} {'Siblings per comet':<18} {'Light-time (yr)':<15} {'Phase-time (yr)':<18}"
    )
    for d in oort_test_distances:
        siblings = compute_comet_sibling_count(d, year_tropical, tidal_exp)
        light_time = compute_light_time_depth(d)
        phase_time = compute_phase_time_depth(
            d, year_tropical, tidal_exp, T_coh="local"
        )
        multiplicity_dominant = "***" if siblings > 1.0 else ""
        print(
            f"{d:<15,} {siblings:<18.2f} {light_time:<15.4f} {phase_time:<18,.0f} {multiplicity_dominant}"
        )
    print()
    ...
    phase_time_gc_hubble = compute_phase_time_depth(
        galactic_center_distance_AU, year_tropical, tidal_exp, T_coh=None
    )
    phase_time_gc_local = compute_phase_time_depth(
        galactic_center_distance_AU, year_tropical, tidal_exp, T_coh="local"
    )
    print(
        f"Galactic center distance:           {galactic_center_distance_ly:,} ly = {galactic_center_distance_AU:,.0f} AU"
    )
    print(f"Light-time depth:                   {light_time_gc:,.0f} years")
    print(
        f"Phase at galactic center:          {effects_gc['phase_fraction_of_pi']:.3f} pi"
    )
    ...
    print(
        f"Expected delay (if Sun's past):     {phase_time_gc_local / 26000:.1f}x present age"
    )
    print()
    ...
    if SHOW_DIAGNOSTICS:
        ...
        print(
            "Unity and balance proxies approximate L²(S²) expectations. Interpret qualitatively."
        )
        print(
            f"{'Distance (AU)':<15} {'Phase/pi':<10} {'U_proxy':<12} {'B_proxy':<12} {'U_proxy vs e^-φ':<18}"
        )
        for d in oort_test_distances:
            ...
            print(
                f"{d:<15,} {phase/math.pi:<10.3f} {u_proxy:<12.4f} {b_proxy:<12.4f} {comparison:<18}"
            )
        print()
        ...
        print(
            f"{'Distance (AU)':<15} {'Coherent (ℓ=0)':<18} {'Incoherent (ℓ>0)':<20} {'Coherent (sr)':<15} {'Incoherent (sr)':<15}"
        )
        for d in oort_test_distances:
            budget = compute_qg_budget(d, year_tropical, tidal_exp)
            print(
                f"{d:<15,} {budget['coherent_fraction']:<18.4f} {budget['incoherent_fraction']:<20.4f} {budget['coherent_solid_angle']:<15.4f} {budget['incoherent_solid_angle']:<15.4f}"
            )
        print()

        spectrum = compute_fourier_spectrum(
            max_distance_ly=5.0, samples=512, tidal_exponent=tidal_exp
        )
        print("14. FOURIER ANALYSIS OF COHERENCE (diagnostic)")
        print(f"{'Frequency (1/ly)':<20} {'Magnitude':<15}")
        for component in spectrum:
            print(
                f"{component['frequency_cycles_per_ly']:<20.6f} {component['magnitude']:<15.6f}"
            )
        print(
            "(Exponential profile dominated by low-frequency components; interpret cautiously.)"
        )
        print()
    ...
    print(
        f"{'Distance (ly)':<15} {'Distance (AU)':<15} {'Phase/pi':<10} {'Coherence':<12} {'Siblings':<10} {'Light-time (yr)':<15} {'Phase-time (yr)':<15}"
    )
    for d_ly in test_distances_ly:
        ...
        phase_time = compute_phase_time_depth(
            d_AU, year_tropical, tidal_exp, T_coh="local"
        )
        print(
            f"{d_ly:<15.1f} {d_AU:<15,.0f} {effects['phase_fraction_of_pi']:<10.3f} {effects['coherence']:<12.4f} {siblings:<10.2f} {light_time:<15.4f} {phase_time:<15,.0f}"
        )
    ...
    print(
        f"{'Closure':<20} {'Distance (ly)':<15} {'Distance (AU)':<15} {'Phase (rad)':<12}"
    )
    for label, data in periods.items():
        print(
            f"{label:<20} {data['distance_ly']:<15.4f} {data['distance_AU']:<15.0f} {data['phase']:<12.3f}"
        )

    closures = find_monodromy_closures(
        max_multiple=40, tidal_exponent=tidal_exp, max_distance_ly=5.0
    )
    print()
    ...
    for entry in closures:
        print(
            f"{entry['multiple']:<5} {entry['phase']:<15.6f} {entry['distance_ly']:<15.6f} {entry['distance_AU']:<15.0f}"
        )
    ...
    print(
        f"{'Threshold':<20} {'n':<5} {'Closure (ly)':<15} {'Target (ly)':<15} {'% diff':<10}"
    )
    for m in matches:
        print(
            f"{m['label']:<20} {m['n']:<5} {m['closure_distance_ly']:<15.6f} {m['target_distance_ly']:<15.6f} {m['pct_diff']:<10.3f}"
        )
    ...
    print(
        f"{'Threshold':<30} {'k-factor':<12} {'Distance (m)':<15} {'Distance (AU)':<15} {'Phase/(4π)':<12} {'Spin proxy':<12}"
    )
    for name, data in micro_e.items():
        ...
        print(
            f"{name:<30} {data['k_factor']:<12.6f} {data['distance_m']:<15.3e} {data['distance_AU']:<15.3e} {phase_micro/(4*math.pi):<12.3f} {spin:<12.3f}"
        )
    print()
    ...
    print(
        "Nested CGM cycles tie microscopic SU(2) behavior to macroscopic phase closure."
    )
