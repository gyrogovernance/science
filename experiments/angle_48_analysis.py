...
class Angle48Analyzer:
    ...
    def analyze_angle_relationships(self) -> Dict:
        ...
        for angle in self.key_angles_deg:
            ...
            results["key_angle_analysis"][f"{angle}°"] = {
                ...
                "fraction_48_over_angle": Fraction(
                    int(round(self.base_angle)), int(round(angle))
                ),
                "fraction_angle_over_48": Fraction(
                    int(round(angle)), int(round(self.base_angle))
                ),
            }
        ...
        for i, angle_rad in enumerate(self.pi_angles_rad):
            ...
            results["pi_angle_analysis"][f"{angle_rad/math.pi:.2f}π"] = {
                ...
                "percentage_difference": pct_diff,
                "fraction_48_over_angle": Fraction(
                    int(round(self.base_angle)), int(round(angle_deg))
                ),
                "fraction_angle_over_48": Fraction(
                    int(round(angle_deg)), int(round(self.base_angle))
                ),
            }
    ...
    def calculate_geometric_properties(self) -> Dict:
        ...
        return {
            "regular_polygon_sides": 360
            / self.base_angle,  # How many sides for regular polygon with 48° interior angle
            "exterior_angle": 180 - self.base_angle,  # Exterior angle
            "interior_angle_sum": (360 / self.base_angle - 2)
            * 180,  # Sum of interior angles
            "central_angle": self.base_angle,  # Central angle in circle
            "arc_length_ratio": self.base_angle
            / 360,  # Arc length as fraction of circumference
            "sector_area_ratio": self.base_angle
            / 360,  # Sector area as fraction of circle
            "chord_length_ratio": 2
            * math.sin(self.base_angle_rad / 2),  # Chord length as fraction of diameter
            "apothem_ratio": math.cos(
                self.base_angle_rad / 2
            ),  # Apothem as fraction of radius
        }
    ...
    def generate_summary_table(self, results: Dict) -> str:
        ...
        output.append("=" * 80)
        output.append(
            f"Base angle: {self.base_angle}° = {self.base_angle_rad:.6f} radians"
        )
        output.append("")
        ...
        output.append("-" * 50)
        output.append(
            f"{'Angle':<8} {'Ratio 48°/A':<12} {'Ratio A/48°':<12} {'% Diff':<10} {'Fraction':<15}"
        )
        output.append("-" * 50)
        ...
        for angle_name, data in results["key_angle_analysis"].items():
            ...
            output.append(
                f"{angle_name:<8} {ratio_48_to_a:<12} {ratio_a_to_48:<12} {pct_diff:<10} {fraction:<15}"
            )
        ...
        output.append("-" * 50)
        output.append(
            f"{'Angle':<10} {'Degrees':<10} {'Ratio 48°/A':<12} {'Ratio A/48°':<12} {'% Diff':<10}"
        )
        output.append("-" * 50)
        ...
        for angle_name, data in results["pi_angle_analysis"].items():
            ...
            output.append(
                f"{angle_name:<10} {degrees:<10} {ratio_48_to_a:<12} {ratio_a_to_48:<12} {pct_diff:<10}"
            )
    ...
    def run_complete_analysis(self):
        ...
        for interval, data in harmonics.items():
            if data["closest_whole_number"] > 0:
                print(
                    f"{interval:<15}: {data['closest_whole_number']} intervals fit in 48° "
