...
class CGMAnalysis:
    ...
    def __init__(self):
        ...
        self.experimental_masses = {
            "up": (
                ...
                sqrt2 * 1.27 / v_ew,
                sqrt2 * 0.0022 / v_ew,
            ),  # top, charm, up (Yukawa couplings)
            "down": (
                sqrt2 * 4.18 / v_ew,
                sqrt2 * 0.095 / v_ew,
                sqrt2 * 0.00475 / v_ew,
            ),  # bottom, strange, down (Yukawa couplings)
            "lepton": (
                sqrt2 * 1.77682 / v_ew,
                sqrt2 * 0.105658 / v_ew,
                sqrt2 * 0.000510998 / v_ew,
            ),  # tau, muon, electron (Yukawa couplings)
        }
    ...
    def compute_invariants(self, structure: Dict) -> Dict:
        ...
        for stage in stages:
            vals = {
                s: structure[s][stage]["flavour_1"]["rho"]
                for s in ["up", "down", "lepton"]
            }
            vertical[stage] = {
                ...
                "expected_down/up": (
                    (P["down"] / P["up"]) if stage not in ("BU", "TOP") else 1.0
                ),  # no projector on shell
                "expected_lepton/up": (
                    (P["lepton"] / P["up"]) if stage not in ("BU", "TOP") else 1.0
                ),  # no projector on shell
                "expected_lepton/down": (
                    (P["lepton"] / P["down"]) if stage not in ("BU", "TOP") else 1.0
                ),  # no projector on shell
            }
        return {
    ...
    def interpret_results(self, spins: Dict, patterns: Dict, sector: str) -> None:
        ...
        if abs(clicks_e21) < 0.2:
            print(
                f"          Note: {sector} sector near exact 45° orthogonality (e21 ≈ {clicks_e21:.2f} clicks)"
            )
        elif abs(clicks_e21 - 1.0) < 0.3:
            print(
                f"          Note: {sector} sector ~1 aperture click above 45° (e21 ≈ {clicks_e21:.2f} clicks)"
            )
        elif abs(clicks_e21 - 2.0) < 0.5:
            print(
                f"          Note: {sector} sector ~2 aperture clicks above 45° (e21 ≈ {clicks_e21:.2f} clicks)"
            )
        elif abs(clicks_e21 - 3.0) < 0.5:
            print(
                f"          Note: {sector} sector ~3 aperture clicks above 45° (e21 ≈ {clicks_e21:.2f} clicks)"
            )

        # 30-fold structure (π/60 fine structure)
        if patterns["residual_30fold"] < self.CLEAN_PHASE_THRESHOLD:
            print(
                f"          Note: {sector} sector exhibits clean 30-fold phase division"
            )
        ...
        if abs(impact_e21 - 1.0) > 0.01:  # Significant aperture impact
            print(
                f"          Note: 3° aperture shifts mass ratio by factor {impact_e21:.3f}"
            )
    ...
    def analyze_geometric_patterns(self, e32: float, e21: float) -> Dict:
        ...
        return {
            "ratio_minus_16_15": (
                abs((e32 / e21) - 16 / 15) if e21 != 0 else float("inf")
            ),
            "deviation_from_16_15": (
                abs((e32 / e21) - 16 / 15) if e21 != 0 else float("inf")
            ),
    ...
    def run(self, rho_union: Optional[List[float]] = None) -> None:
        ...
        print(
            f"K = {self.c.optical_invariant:.2e} GeV², E_MID = {self.c.fixed_point:.2e} GeV"
        )
        ...
        for sector in ["up", "down", "lepton"]:
            ...
            print(
                f"  {sector:7s}: {ratio:.6f} (expected {exp:.6f}) {'OK' if abs(ratio-exp) < self.TOL else 'FAIL'}"
            )
        ...
        for stage in ["BASE", "MID", "BU", "UNION", "UNA", "ONA"]:
            ...
            ok = (
                abs(v["down/up"] - v["expected_down/up"]) < self.TOL
                and abs(v["lepton/up"] - v["expected_lepton/up"]) < self.TOL
                and abs(v["lepton/down"] - v["expected_lepton/down"]) < self.TOL
            )
        ...
        for sector in ["up", "down", "lepton"]:
            vals = [
                structure[sector][s]["flavour_1"]["rho"]
                for s in ["BASE", "MID", "BU", "UNION", "UNA", "ONA"]
            ]
            print(
                f"{sector:7s} {vals[0]:.3e}  {vals[1]:.3e}  {vals[2]:.3e}  {vals[3]:.3e}  {vals[4]:.3e}  {vals[5]:.3e}"
            )
        ...
        for sector in ["up", "down", "lepton"]:
            ...
            print(
                f"  {sector:7s}: c = {vals} (target {seesaw[sector]['target']:.6f}) {'OK' if seesaw[sector]['ok'] else 'FAIL'}"
            )

        print("\nSterile neutrino inference:")
        for row in self.infer_sterile_parameters([0.001, 0.01, 0.05]):
            print(
                f"  mν={row['m_light_eV']:.3e} eV → yv={row['y_v_GeV']:.2f} GeV, M_R={row['M_R_GeV']:.2e} GeV"
            )
        ...
        for sector in ["up", "down", "lepton"]:
            ...
            print(f"\n{sector:7s} sector:")
            print(
                f"  e-exponents: e32={s['e32']:.6f}, e21={s['e21']:.6f}, e13={s['e13']:.6f}"
            )
            print(
                f"  45° lattice: e32≈{g['nearest_45_e32']:.3f}, e21≈{g['nearest_45_e21']:.3f}"
            )
            print(
                f"  3° aperture clicks: e32={g['clicks_e32']:.2f}, e21={g['clicks_e21']:.2f}"
            )
            print(
                f"  Aperture impact: e32×{g['aperture_impact_e32']:.3f}, e21×{g['aperture_impact_e21']:.3f}"
            )
            m2_m3_actual = m2 / m3
            m2_m3_reconstructed = s["m2_m3_reconstructed"]
            percent_error = abs(m2_m3_actual - m2_m3_reconstructed) / m2_m3_actual * 100
            print(
                f"  Mass ratios: actual m2/m3={m2_m3_actual:.6f}, reconstructed m2/m3={m2_m3_reconstructed:.6f} (error: {percent_error:.2e}%)"
            )
            print(f"               actual m2/m1={m2/m1:.1f}")
            print(
                f"  Vertical-UNI diagnostic: K_gen/K_expected = {v_conj['ratio']:.3f} (target 1.0)"
            )
