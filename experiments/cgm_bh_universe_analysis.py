...
def analyze_sgra_horizon():
    ...
    f_plasma = (1.0 / (2.0 * math.pi)) * math.sqrt(
...
def test_ir_anchor_sensitivity():
    ...
    return {"K_QCD": K_QCD, "energies_ir_qcd": energies_ir_qcd, "E_QCD": E_QCD}
...
def print_stage_table(energies_uv, energies_ir, dual_invariants):
    ...
    print(
        f"{'Stage':<6} {'E_UV (GeV)':<12} {'E_IR (GeV)':<12} {'r_s_UV (m)':<12} {'r_s_IR (m)':<12} {'S_UV (k_B)':<12} {'S_IR (k_B)':<12}"
    )
    print("-" * 90)
    ...
    for stage in energies_uv:
        ...
        print(
            f"{stage:<6} {energies_uv[stage]:<12.2e} {energies_ir[stage]:<12.2e} {r_s_uv:<12.2e} {r_s_ir:<12.2e} {S_uv:<12.2e} {S_ir:<12.2e}"
        )
...
def main():
    ...
    print(
        "CGM BLACK HOLE UNIVERSE: Complete Framework Test with Observational Validation"
    )
    print("=" * 90)
    ...
    print(
        f"  Quantum Gravity: Q_G = {identities['Q_G']:.6f} steradians (complete observational solid angle)"
    )
    ...
    print(
        f"  Note: Q_G = 4π defines quantum gravity geometrically, not as a quantized field"
    )
    ...
    for stage in energies_uv:
        ...
        print(
            f"  {stage:3s}: {energies_uv[stage]:.2e} × {energies_ir[stage]:.2e} = {conjugacy_check[stage]:.6e} GeV² (δ: {deviation:.2e})"
        )
    ...
    print(
        f"  Schwarzschild radius product (all stages): {list(dual_invariants['r_s_products'].values())[0]:.6e} m²"
    )
    print(
        f"  Theoretical r_s product: {dual_invariants['theoretical_rs_product']:.6e} m²"
    )
    print(
        f"  Dual length L_dual = √(r_s,UV × r_s,IR): {dual_invariants['L_dual']:.6e} m"
    )
    ...
    print(
        f"  CGM enhancement factor: {S_prod_CGM/S_prod_GR:.6f} (exact: {ratio_expected:.6f})"
    )
    print(
        f"  Theoretical entropy product: {dual_invariants['theoretical_entropy_product']:.6e}"
    )

    # Universe as black hole - both Planck and SH0ES
    universe_planck = analyze_universe_as_blackhole(
        H0_PLANCK, H0_PLANCK_ERR, "Planck 2023"
    )
    universe_shoes = analyze_universe_as_blackhole(H0_SHOES, H0_SHOES_ERR, "SH0ES 2022")

    print(f"\nUniverse as Critical Black Hole:")
    print(
        f"  Planck 2023: H₀ = {H0_PLANCK*1e18:.3f} ± {H0_PLANCK_ERR*1e18:.3f} × 10⁻¹⁸ s⁻¹"
    )
    print(
        f"    r_s/R_H = {universe_planck['ratio_rs_R']:.4f} ± {universe_planck['ratio_rs_R_err']:.4f}"
    )
    print(f"    De Sitter temperature: {universe_planck['T_dS']:.3e} K")
    print(
        f"    De Sitter entropy: {universe_planck['S_dS']:.2e} ± {universe_planck['S_dS_err']:.2e} k_B"
    )

    print(
        f"  SH0ES 2022:  H₀ = {H0_SHOES*1e18:.3f} ± {H0_SHOES_ERR*1e18:.3f} × 10⁻¹⁸ s⁻¹"
    )
    print(
        f"    r_s/R_H = {universe_shoes['ratio_rs_R']:.4f} ± {universe_shoes['ratio_rs_R_err']:.4f}"
    )
    print(f"    De Sitter temperature: {universe_shoes['T_dS']:.3e} K")
    print(
        f"    De Sitter entropy: {universe_shoes['S_dS']:.2e} ± {universe_shoes['S_dS_err']:.2e} k_B"
    )
    ...
    for a_star in [0.0, 0.5, 0.9]:
        ...
        print(
            f"  a* = {a_star:.1f}: A_UV × A_IR = {area_product:.6e} m⁴ (stage scatter: {scatter:.2e})"
        )

    # Spin dependence ratio
    area_ratio = (
        list(kerr_results[0.9].values())[0] / list(kerr_results[0.0].values())[0]
    )
    ...
    print(f"\nBoundary Scale Analysis:")
    print(
        f"  Planck boundary: M = {cs_props['mass_kg']:.3e} kg, r_s = {cs_props['r_s']:.3e} m, S = {cs_props['entropy_kb_units']:.2e} k_B"
    )
    print(
        f"  EW boundary (horizon-like closure): M = {ew_props['mass_kg']:.3e} kg, r_s = {ew_props['r_s']:.3e} m, S = {ew_props['entropy_kb_units']:.2e} k_B"
    )
    ...
    print(
        f"  Frequency ratio f_H/f_p: {sgra['ratio_f']:.3e} (EM radiation blocked by ISM)"
    )
    ...
    print(
        f"  Observed BH density ρ_BH: {ccbh['rho_BH_obs']:.2e} kg/m³ (JWST+UNCOVER 2024)"
    )
    print(f"  CCBH predicted ρ_DE: {ccbh['rho_DE_BH']:.2e} kg/m³ (k = {K_CCBH})")
    print(f"  Observed dark energy ρ_DE: {ccbh['rho_de_obs']:.2e} kg/m³")
    print(
        f"  CCBH/observed ratio: {ccbh['ratio_ccbh_obs']:.3e} (short by ~{ccbh['deficit_factor']:.0e}×)"
    )
    print(
    ...
    print(
        f"  • Universe r_s/R_H ratio: {universe_planck['ratio_rs_R']:.4f} ± {universe_planck['ratio_rs_R_err']:.4f} (Planck)"
    )
    ...
    print(
        f"  H0 ratio predicted (20% void × aperture): {hdiag['ratio_pred_20pct_void']:.4f}"
    )
    print(
    ...
    print(
        f"  Redshift drift at z=2 (ΛCDM): {rd['dv_dt_LCDM_cm_per_s_per_yr']:.3f} cm/s/yr  vs CGM (no-expansion): {rd['dv_dt_CGM_cm_per_s_per_yr']:.3f}"
    )

    dd = distance_duality_prediction()
    print(
        f"  Distance duality deviation η0 (predicted): {dd['eta0_range'][0]:.3f} – {dd['eta0_range'][1]:.3f}"
    )

    gw = gw_memory_prediction()
    print(
        f"  GW memory fraction h_mem/h_peak (predicted): {gw['h_memory_over_h_peak']*100:.2f}%"
    )

    w0 = effective_w0_from_q0(q0=-0.55, Omega_m=0.315)
    print(
        f"  Effective w0 from q0,Ωm (diagnostic): {w0['w_eff0_from_q0']:.3f}  (≈ −1 expected observationally)"
    )

    okb = apparent_curvature_bound(
        universe_planck["ratio_rs_R"], universe_planck["ratio_rs_R_err"]
    )
    print(
        f"  Apparent curvature bound proxy |Ω_k,eff| ≲ {okb['Omega_k_eff_bound_1sigma']:.2e} (very conservative)"
    )
...
def redshift_drift_LCDM_vs_CGM(z=2.0, Omega_m=0.315):
    ...
    dv_dt_LCDM = (
        (C * dz_dt_LCDM / (1 + z)) * (365.25 * 24 * 3600) * 100.0
    )  # m/s -> cm/s per year
