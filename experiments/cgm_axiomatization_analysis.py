...
from z3 import Solver, Bool, BoolVal, And, Or, Not, Implies, sat, unsat, is_true, BoolRef
...
OA_TO_CANON = {'OA1': 'CS', 'OA2': 'UNA', 'OA3': 'ONA', 'OA4': 'BU_EGRESS', 'OA5': 'BU_INGRESS'}
CANON_TO_ATOMIC = {'CS': 'A1', 'UNA': 'A2', 'ONA': 'A3', 'BU_EGRESS': 'A4', 'BU_INGRESS': 'A5'}
...
def canonicalize(token: str) -> str:
    ...
    if token.startswith('OA') and token in OA_TO_CANON:
        warnings.warn(f"{token} is deprecated; use canonical tokens (CS, UNA, ONA, BU_EGRESS, BU_INGRESS).", DeprecationWarning)
...
class ConstraintEncoder:
    ...
    def B_at(self, w: int) -> BoolRef:
        ...
        LRLR = self.box_L(lambda w1:
                    self.box_R(lambda w2:
                        self.box_L(lambda w3:
                            self.box_R(lambda w4: self.S[w4])(w3)
                        )(w2)
                    )(w1)
                )(w)
        RLRL = self.box_R(lambda w1:
                    self.box_L(lambda w2:
                        self.box_R(lambda w3:
                            self.box_L(lambda w4: self.S[w4])(w3)
                        )(w2)
                    )(w1)
                )(w)
    ...
    def encode_S_generated(self, s: Solver) -> None:
        ...
        reach = [[Bool(f'reach_{k}_{w}') for w in range(self.n)] for k in range(self.n + 1)]
        for w in range(self.n):
        ...
        for k in range(self.n):
            for w in range(self.n):
                s.add(reach[k + 1][w] ==
                      Or(reach[k][w],
                         Or([And(reach[k][prev], Or(self.R_L[prev][w], self.R_R[prev][w]))
    ...
    def extract_frame(self, model: Any) -> Dict[str, Any]:
        ...
        frame = {
            ...
            'R_L': {w: [v for v in range(self.n) if is_true(model[self.R_L[w][v]])] 
                    for w in range(self.n)},
            'R_R': {w: [v for v in range(self.n) if is_true(model[self.R_R[w][v]])] 
                    for w in range(self.n)},
...
def test_consistency(constraints: List[str], n: int = 3, timeout: int = 10000, 
                    generated: bool = False) -> Optional[Dict[str, Any]]:
...
def test_entailment(premises: List[str], conclusion: str, n: int = 4, 
                   timeout: int = 10000, generated: bool = False) -> bool:
...
def test_independence(constraint: str, others: List[str], n: int = 4, 
                     timeout: int = 10000, generated: bool = False) -> Tuple[Optional[str], Optional[Dict[str, Any]]]:
...
def verify_frame_locally(frame: Dict[str, Any], constraint: str) -> bool:
    ...
    def B_at_py(w: int) -> bool:
        LRLR = box_L_py(lambda w1:
                 box_R_py(lambda w2:
                   box_L_py(lambda w3:
                     box_R_py(lambda w4: w4 in S_set, w3), w2), w1), w)
        RLRL = box_R_py(lambda w1:
                 box_L_py(lambda w2:
                   box_R_py(lambda w3:
                     box_L_py(lambda w4: w4 in S_set, w3), w2), w1), w)
        return LRLR == RLRL
...
def test_entailment_atomics(atomics: List[str], target: str, n: int = 3, 
                            timeout: int = 10000, generated: bool = False) -> bool:
...
def find_minimal_entailing_subsets(target: str, all_constraints: List[str], 
                                   n: int = 3, timeout: int = 10000, 
                                   use_atomics: bool = True) -> List[List[str]]:
...
def find_non_commutation_witness(n: int = 4, timeout: int = 10000) -> Optional[Dict[str, Any]]:
...
def test_oa4_not_collapse_LR(n: int = 4, timeout: int = 10000) -> Optional[Dict[str, Any]]:
    ...
    for w in range(n):
        s.add(Implies(oa.S[w], Or([
            Not(oa.R_L[w][v] == oa.R_R[w][v])
            for v in range(n)
        ])))
...
def test_entailment_E(constraint: str, target: str, n: int = 4, timeout: int = 10000) -> bool:
    ...
    if target == 'not_E':
        ...
        s.add(Or([And(oa.S[w], oa.box(lambda v: cast(BoolRef, Not(oa.E_at(v))))(w)) for w in range(n)]))
...
def main() -> None:
    ...
    print("CGM FOUNDATIONAL CONSTRAINTS TEST SUITE (CS, UNA, ONA, BU‑Egress, BU‑Ingress)")
    ...
    forward_tests = [
        ...
        (['CS', 'UNA', 'ONA', 'BU_EGRESS'], 'BU_EGRESS', 'Full forward chain to BU‑Egress')
    ]
    ...
    independence_tests = [
        ...
        ('CS', ['UNA', 'ONA', 'BU_EGRESS', 'BU_INGRESS'], 'CS from UNA/ONA/BU‑Egress/BU‑Ingress'),
        ('BU_EGRESS', ['BU_INGRESS'], 'BU‑Egress independent of BU‑Ingress')
    ]
    for constraint, others, desc in independence_tests:
        status, witness = test_independence(constraint, others, n=3 if constraint in ['UNA', 'ONA'] else 4)
    ...
    print(f"BU‑Ingress alone → BU‑Egress: {bu_ingress_alone_egress} (BU‑Ingress does not imply closure)")
    
    if not bu_egress_alone_cs and bu_dual_cs:
        print("BU‑Ingress (A5) is necessary for reconstruction: BU‑Egress alone cannot reconstruct CS")
    ...
    gen_frame = test_consistency(['CS', 'UNA', 'ONA', 'BU_EGRESS', 'BU_INGRESS'], 
                                 n=4, generated=True)
    if gen_frame:
    ...
    for target in ['UNA', 'ONA', 'BU_EGRESS', 'BU_INGRESS', 'BU_DUAL']:
        minimal = find_minimal_entailing_subsets(target, ['CS', 'UNA', 'ONA', 'BU_EGRESS', 'BU_INGRESS'], 
                                                 n=3, use_atomics=True)
    ...
    if frame:
        ...
        print(f"CS + BU_EGRESS → ONA: {test_entailment(['CS', 'BU_EGRESS'], 'ONA', n=3)}")
        print(f"CS + BU_EGRESS → UNA: {test_entailment(['CS', 'BU_EGRESS'], 'UNA', n=3)}")
    else:
    ...
    exists_diff = Or([Or([Not(oa.R_L[w][v] == oa.R_R[w][v]) for v in range(4)]) for w in range(4)])
    s.add(exists_diff)
    if s.check() == sat:
