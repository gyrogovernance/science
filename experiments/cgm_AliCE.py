...
chi_eV, _ = compute_chi(E_UNA_UV * GeV_to_eV, E_UNA_IR * GeV_to_eV,
                         E_CS * GeV_to_eV, E_BU * GeV_to_eV)
...
def reciprocity_check(chi_fn, N=1000):
    ...
    for _ in range(N):
        ...
        if isinstance(chi1, tuple): chi1 = chi1[0]
        if isinstance(chi2, tuple): chi2 = chi2[0]
...
def independence_check(chi_fn, N=500):
    ...
    for _ in range(N):
        ...
        if isinstance(chi1, tuple): chi1 = chi1[0]
        if isinstance(chi2, tuple): chi2 = chi2[0]
        if isinstance(chi12, tuple): chi12 = chi12[0]
