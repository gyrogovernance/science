#!/usr/bin/env python3
"""
hqvm_corrections_analysis_2.py
...
class CGMRotationCurves:
    ...
    def __init__(self):
        ...
        self.a0_planck, self.a0_planck_err = self._compute_a0_with_errors(
            self.H0_planck, self.H0_planck_err
        )
        self.a0_sh0es, self.a0_sh0es_err = self._compute_a0_with_errors(
            self.H0_sh0es, self.H0_sh0es_err
        )

        print(f"CGM Initialized:")
        print(
            f"  Aperture fraction Δ = {self.Delta:.6f} ({float(self.Delta)*100:.3f}%)"
        )
        print(f"  a0 (Planck) = {self.a0_planck:.6e} ± {self.a0_planck_err:.6e} m/s²")
        print(f"  a0 (SH0ES) = {self.a0_sh0es:.6e} ± {self.a0_sh0es_err:.6e} m/s²")

    def _compute_a0_with_errors(
        self, H0_km_s_Mpc: Decimal, H0_err_km_s_Mpc: Decimal
    ...
    def baryonic_velocity_curve(
        self, df: pd.DataFrame, ups_disk: float = 0.5, ups_bulge: float = 0.7
        ...
        Vb = np.sqrt(
            np.maximum(0.0, Vgas**2 + (ups_disk * Vdisk) ** 2 + (ups_bulge * Vbul) ** 2)
        )

        return R, Vb

    def newtonian_velocity(
        self, r_kpc: np.ndarray, v_baryon_km_s: np.ndarray
    ) -> np.ndarray:
        """Newtonian velocity is just the baryonic velocity (no dark matter)."""
        return v_baryon_km_s

    def mond_velocity(
        self, r_kpc: np.ndarray, v_baryon_km_s: np.ndarray, a0: float
    ) -> np.ndarray:
    ...
    def nfw_dark_matter_velocity(
        self, r_kpc: np.ndarray, v_baryon_km_s: np.ndarray, M_baryon_solar: float
    ) -> np.ndarray:
        ...
        v_nfw_squared = (6.67e-11 * M_halo_kg * f_x) / (
            r_m * (np.log(1 + 1) - 1 / (1 + 1))
        )  # Normalized
    ...
    def cgm_velocity(
        ...
        # C_IDE = 1 + (1/ρ) * diff * Δ⁴
        c_ide = float(
            1 + float(self.rho_inv) * float(self.diff) * (float(self.Delta) ** 4)
        )
        ...
        for file_path in data_files:
            try:
                ...
                if df.shape[1] >= 6:
                    ...
                    if missing_cols:
                        excluded += 1
                        exclusion_reasons["missing_columns"] = (
                            exclusion_reasons.get("missing_columns", 0) + 1
                        )
                        continue
                    ...
                    df_clean["e_Vobs"] = pd.to_numeric(
                        df_clean["e_Vobs"], errors="coerce"
                    )
                    ...
                    df_clean = df_clean[
                        df_clean["Vobs"] > 0
                    ]  # Positive velocities only

                    if (
                        len(df_clean) > 5
                    ):  # Only include galaxies with enough data points
                        galaxy_name = os.path.basename(file_path).replace(
                            "_rotmod.dat", ""
                        )
                        ...
                        exclusion_reasons["insufficient_data"] = (
                            exclusion_reasons.get("insufficient_data", 0) + 1
                        )
                else:
                    excluded += 1
                    exclusion_reasons["insufficient_columns"] = (
                        exclusion_reasons.get("insufficient_columns", 0) + 1
                    )

            except Exception as e:
                excluded += 1
                exclusion_reasons["load_error"] = (
                    exclusion_reasons.get("load_error", 0) + 1
                )
    ...
    def print_galaxy_analysis(self, analysis: Dict):
        ...
        print(f"\n=== {analysis['name']} ===")
        print(
            f"Data: {len(analysis['r'])} points, R={analysis['r'].min():.1f}-{analysis['r'].max():.1f} kpc, V={analysis['v_obs'].min():.0f}-{analysis['v_obs'].max():.0f} km/s"
        )
        print(f"Mass: {analysis['M_baryon']:.1e} M☉")
        print(
            f"χ²: Newton={analysis['chi2_newton']:.0f}, MOND={analysis['chi2_mond']:.0f}, DM={analysis['chi2_dark_matter']:.0f}, CGM={analysis['chi2_cgm']:.0f}"
        )
        print(
            f"Reduced χ²: Newton={analysis['rchi2_newton']:.2f}, MOND={analysis['rchi2_mond']:.2f}, DM={analysis['rchi2_dark_matter']:.2f}, CGM={analysis['rchi2_cgm']:.2f}"
        )
        print(
            f"AIC: Newton={analysis['aic_newton']:.1f}, MOND={analysis['aic_mond']:.1f}, DM={analysis['aic_dark_matter']:.1f}, CGM={analysis['aic_cgm']:.1f}"
        )
        print(
            f"BIC: Newton={analysis['bic_newton']:.1f}, MOND={analysis['bic_mond']:.1f}, DM={analysis['bic_dark_matter']:.1f}, CGM={analysis['bic_cgm']:.1f}"
        )
        ...
        print(
            f"CGM base operator: C_AB={c_ab_spin2:.6f}, C_HC={c_hc:.6f}, C_IDE={c_ide:.6f}, C_base={c_base:.6f}"
        )
        print(
            f"  f_x range: {fx.min():.6f}–{fx.max():.6f} (median {np.median(fx):.6f})"
        )
    ...
    def plot_galaxy(self, galaxy_name: str, analysis: Dict, save_plot: bool = True):
        ...
        if save_plot:
            plt.savefig(
                f"plots/{galaxy_name}_rotation_curve.png", dpi=300, bbox_inches="tight"
            )
    ...
    def plot_comparison(self, results: List[Dict], save_plot: bool = True):
        ...
        width = 0.25
        axes[0, 0].bar(
            x - width, chi2_newton, width, label="Newton", color="red", alpha=0.7
        )
        axes[0, 0].bar(x, chi2_mond, width, label="MOND", color="blue", alpha=0.7)
        axes[0, 0].bar(
            x + width, chi2_cgm, width, label="CGM", color="green", alpha=0.7
        )
        ...
        axes[0, 1].bar(
            x - width, rchi2_newton, width, label="Newton", color="red", alpha=0.7
        )
        axes[0, 1].bar(x, rchi2_mond, width, label="MOND", color="blue", alpha=0.7)
        axes[0, 1].bar(
            x + width, rchi2_cgm, width, label="CGM", color="green", alpha=0.7
        )
        ...
        axes[1, 0].hist(
            cgm_mond_ratios, bins=20, alpha=0.7, color="purple", edgecolor="black"
        )
        axes[1, 0].axvline(
            np.mean(cgm_mond_ratios),
            color="red",
            linestyle="--",
            linewidth=2,
    ...
    def run_analysis(
        ...
        for i, name in enumerate(galaxy_names):
            ...
            if n_galaxies > 10 and (i + 1) % 20 == 0:
                ...
                print(
                    f"{name}: M={analysis['M_baryon']:.1e} M☉, χ²: N={analysis['chi2_newton']:.0f}, M={analysis['chi2_mond']:.0f}, DM={analysis['chi2_dark_matter']:.0f}, C={analysis['chi2_cgm']:.0f}"
                )
        ...
        print(
            f"Newton (no DM):  χ² = {np.mean(chi2_newton):.0f} ± {np.std(chi2_newton):.0f}, χ²/dof = {np.mean(rchi2_newton):.2f}"
        )
        print(
            f"MOND:           χ² = {np.mean(chi2_mond):.0f} ± {np.std(chi2_mond):.0f}, χ²/dof = {np.mean(rchi2_mond):.2f}"
        )
        print(
            f"Dark Matter:    χ² = {np.mean(chi2_dark_matter):.0f} ± {np.std(chi2_dark_matter):.0f}, χ²/dof = {np.mean(rchi2_dark_matter):.2f}"
        )
        print(
            f"CGM:            χ² = {np.mean(chi2_cgm):.0f} ± {np.std(chi2_cgm):.0f}, χ²/dof = {np.mean(rchi2_cgm):.2f}"
        )
        ...
        cgm_wins = sum(
            1
            for r in results
            if r["aic_cgm"] < min(r["aic_newton"], r["aic_mond"], r["aic_dark_matter"])
        )
        mond_wins = sum(
            1
            for r in results
            if r["aic_mond"] < min(r["aic_newton"], r["aic_cgm"], r["aic_dark_matter"])
        )
        dark_matter_wins = sum(
            1
            for r in results
            if r["aic_dark_matter"] < min(r["aic_newton"], r["aic_mond"], r["aic_cgm"])
        )
        newton_wins = sum(
            1
            for r in results
            if r["aic_newton"] < min(r["aic_mond"], r["aic_cgm"], r["aic_dark_matter"])
        )
        ...
        print(
            f"  Per-galaxy mean (v_cgm/v_mond - 1): {np.mean(per_galaxy_means):.6f} ± {np.std(per_galaxy_means):.6f}"
        )
        print(
            f"  Per-galaxy RMS (v_cgm/v_mond - 1): {np.mean(per_galaxy_rms):.6f} ± {np.std(per_galaxy_rms):.6f}"
        )
        ...
        print(
            f"  x < 0.1 (deep-MOND): mean = {np.mean(low_acc):.6f}, std = {np.std(low_acc):.6f}, N = {len(low_acc)}"
        )
        print(
            f"  0.1 ≤ x ≤ 10 (transition): mean = {np.mean(mid_acc):.6f}, std = {np.std(mid_acc):.6f}, N = {len(mid_acc)}"
        )
        print(
            f"  x > 10 (Newtonian): mean = {np.mean(high_acc):.6f}, std = {np.std(high_acc):.6f}, N = {len(high_acc)}"
        )
        ...
        for r in results:
            ...
            c_ab_spin2 = float(
                1 - (5.0 / 4.0) * float(self.R) * (float(self.Delta) ** 2)
            )
            phi = 3 * float(self.delta_BU) + float(self.diff)
            phi_term = (phi / (3 * float(self.delta_BU))) - 1
            h_term = 1 - (float(self.Delta) ** 2) * float(self.h)
        ...
        print(
            f"  δ_pred = 0.5(1/C_total - 1): mean = {np.mean(delta_pred_list):.6f} ± {np.std(delta_pred_list):.6f}"
        )
        print(
            f"  δ_meas = mean(v_CGM/v_MOND - 1): mean = {np.mean(delta_meas_list):.6f} ± {np.std(delta_meas_list):.6f}"
        )
        print(
            f"  δ_meas - δ_pred: mean = {np.mean(residual):.6f} ± {np.std(residual):.6f}"
        )
        ...
        print(
            f"Note: No mass-to-light fitting; higher χ² expected; comparisons across models are still fair"
        )
        ...
        print(
            f"  Planck: δa0/a0 = δH0/H0 = {float(self.H0_planck_err/self.H0_planck)*100:.2f}%"
        )
        print(
            f"  SH0ES:  δa0/a0 = δH0/H0 = {float(self.H0_sh0es_err/self.H0_sh0es)*100:.2f}%"
        )
...
def main():
    ...
    parser.add_argument(
        "--a0",
        choices=["planck", "sh0es"],
        default="planck",
        help="H0 source for a0 calculation (default: planck)",
    )
    parser.add_argument(
        "--plots", action="store_true", help="Enable plotting (default: disabled)"
    )
    parser.add_argument(
        "--individual-plots",
        action="store_true",
    ...
    parser.add_argument(
        "--help-plots", action="store_true", help="Show detailed help for plot options"
    )
    ...
    if args.help_plots:
        ...
        print("Examples:")
        print(
            "  python cgm_rotations_analysis.py                    # No plots (default)"
        )
        print(
            "  python cgm_rotations_analysis.py --plots            # Enable all plots"
        )
        print(
            "  python cgm_rotations_analysis.py --individual-plots # Only individual plots"
        )
        print("  python cgm_rotations_analysis.py --all-plots        # All plots")
        print(
            "  python cgm_rotations_analysis.py -g 10 --plots      # 10 galaxies with plots"
        )
        print(
            "  python cgm_rotations_analysis.py --a0 sh0es --plots # Use SH0ES a0 with plots"
        )
        return
